...
**How to apply:** Validate submitted and current values for user feedback, then repeat the comparison against a fresh database read after the component lock and before any component, audit, inherited-child, or stamp write. Sync derivation must preserve higher live values unless the winning event is an approved reset.