...
The compiler was run with emission disabled. The investigation changed only this report. It did not alter application source, `tsconfig.json`, package manifests, lockfiles, shared schema, migrations, database state, dependencies, APIs, sync code, tests, or runtime behavior.# TypeScript Error Baseline Investigation
