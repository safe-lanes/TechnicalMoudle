    ...
    try:
        for f in files:
            ...
            try:
                ...
                # XREF_LAYOUT=quote-excluded: the verbatim source text of each resolved cross-reference
                # was kept OUT of the page text; store it in the owning chunk's metadata for audit and
                # citation. Matched by page and by the "Applies to:" line the resolver writes.
                if a.resolve_xrefs and xrefs_mod.SOURCE_QUOTES:
                    attached = 0
                    for q in xrefs_mod.SOURCE_QUOTES:
                        for c in chunks:
                            if str(c.metadata.get("page_number")) == str(q.get("page")) and f"Applies to: {q['dest']}" in c.text:
                                c.metadata["source_quote"] = {"citation": q["citation"], "describes": q["src_parent"], "text": q["quote"]}
                                attached += 1
                                break
                    log(f"   source quotes kept out of the text and stored in metadata: {attached} of {len(xrefs_mod.SOURCE_QUOTES)}")
                stub = False
                if not chunks:
                ...
                # reviewer, 22-Sep-2026: a per-document "FAILED" that then continues would leave a partial
                # index behind and hide the miss in a summary line. Under --cache-only the whole build stops.
                log(f"   ABORT (--cache-only): {e}")
                raise SystemExit(f"--cache-only: build aborted on {f.name}; nothing further processed") from e
