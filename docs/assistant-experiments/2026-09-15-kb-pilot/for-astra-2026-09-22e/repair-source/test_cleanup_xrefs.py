...
def test_destination_without_verified_facts_says_so_instead_of_inventing():
    ...
    # the note must name its author: three live answers reported "the manual notes that the screen
    # details have not been verified" when the manual says nothing of the kind
    assert "Indexing note (written at index time by the cross-reference resolver, NOT a statement from the manual)" in block
    assert "not been confirmed for Waitlist" in block
    assert "The manual itself makes no such statement" in block
...
def test_quote_excluded_layout_keeps_source_text_out_of_the_page(monkeypatch):
    """Reviewer, 22-Sep-2026: labelling the quote 'audit copy' still lets the old instructions reach the
    model. Under XREF_LAYOUT=quote-excluded the verbatim source text must NOT appear in the page text at
    all; it is handed to the indexer via SOURCE_QUOTES (with its page) for metadata storage."""
    monkeypatch.setenv("XREF_LAYOUT", "quote-excluded")
    xrefs.SOURCE_QUOTES.clear()
    xrefs._CURRENT_PTR.update({"page": 44, "section": "1.1.8.6 HOW TO EXPORT STORE ITEMS"})
    body = "* Click on the 'Spares' sub-sub module.\nscreenshot: Spares Inventory dashboard.\n* Click 'Export'."
    block = xrefs.render_resolved(dest_parent="Stores", dest_title="How To Export Store Items",
                                  src_citation="section 1.1.7.7 'How To Export Spares Records' under Spares, page 41",
                                  src_parent="Spares", body=body, src_title="How To Export Spares Records",
                                  pointer_body="* Refer to the 'Spares' sub-sub-module and apply the same steps.")
    assert "Click on the 'Spares' sub-sub module" not in block          # the old instruction is gone from the text
    assert "Spares Inventory dashboard" not in block
    assert "Click on the 'Stores' sub-sub module" in block             # the adapted step is present
    assert "preserved verbatim in the index record" in block
    assert "quoted verbatim" not in block.lower()      # the intro must not promise a quote that is not there
    assert len(xrefs.SOURCE_QUOTES) == 1
