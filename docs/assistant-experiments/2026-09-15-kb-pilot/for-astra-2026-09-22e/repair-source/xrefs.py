...
def adapt_steps(body: str, src_parent: str, dest_parent: str, dest_title: str, src_title: str,
    ...
    for raw in body.splitlines():
        ...
        if re.search(r"\bfilters?\b", s, re.I) and re.search(r"\bsuch as\b|\bby\b.*,|\(.*,.*\)", s):
            # rule 2 — the enumerated list may be introduced by "such as"/"by" OR sit in parentheses:
            # "dropdown filters (Criticality, Rotation Item, or Stock)". The reviewer reproduced the
            # bug where the detector saw the parentheses but the rewrite handled only the first two
            # forms, so the wrong list survived INSIDE the adapted Stores steps — and the same block
            # then said those filters do not exist. Contradictory guidance, generated by us.
            paren = re.search(r"\(([^()]*,[^()]*)\)", s)
            if f.get("filters"):
                verified = ", ".join(f["filters"])
                if paren:
                    s = s[:paren.start()] + f"({verified})" + s[paren.end():]
                else:
                    s = re.sub(r"(such as|by)\s+[^.]*", f"\\1 {verified}", s, count=1)
            else:
                if paren:
                    s = (s[:paren.start()].rstrip() + s[paren.end():]).replace("  ", " ")
                else:
