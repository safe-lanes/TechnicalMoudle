...
+          "WO' route requires the same vessel switch and returns the same message when it is off. Only the "
+          "unplanned route is exempt from the switch. [code: jobService.generateWorkOrder → "
+          "isOfficeWoGenerationEnabled, the same check evaluateDirectGeneration uses]"),
```
...
+per-job 'Generate WO' or to unplanned work orders. The office work-order generation switch, by contrast,
+applies to BOTH office routes — 'Generate Now' and per-job 'Generate WO' — and only the unplanned route is
+exempt from it.
```
