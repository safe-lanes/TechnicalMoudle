# Reviewer pack — candidate answer test + instrument audit, 21-Sep-2026 (fourth pack)

**New in this pack:** an instrument audit. Six of my findings in this workstream were wrong because I
quoted a number without testing the tool that produced it, and you caught every one. `verify_instruments.py`
now runs before any figure is reported, and each check includes a case the instrument must REJECT.
It confirms the citation checker can actually detect a mislabel (so "579 of 579" is not vacuous), that
the page check separates right page from merely right file, and — most importantly — that the offline
model reproduces the LIVE containers exactly: 34 of 34 questions on module choice and on excerpt counts,
both arms, with the +31 added excerpts confirmed live rather than only offline.
