...
### Instrument audit — new standing rule

Six findings in this workstream were wrong because I quoted a number without testing the tool that
produced it. `verify_instruments.py` now runs before any figure is reported, and every check includes
a case the instrument must **reject** — a checker that cannot fail is not evidence.

| instrument | verified by |
|---|---|
| `cite_module_ok` | returns **false** on a deliberately mislabelled citation, and declines to judge an unknown manual. Without this, "579 of 579 correct" could have been vacuous. |
| `supplied_expected` | separates right page from merely right file: right file + wrong page returns `(file yes, page no)`. This is the check that was wrong last round. |
| the offline `route()` reproduction | **34 of 34** tested questions predict the live baseline container's actual answering module. |
| the offline excerpt model | **34 of 34**, both arms, predict the live containers' actual excerpt counts — and the **+31 added excerpts is confirmed live**, not only offline. |

The last two matter most: they establish that the retrieval table in §2, which is computed offline over
all 101 questions, reproduces what the running system actually does.
