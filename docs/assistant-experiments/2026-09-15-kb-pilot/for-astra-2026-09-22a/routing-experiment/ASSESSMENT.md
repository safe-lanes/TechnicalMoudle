...
No new model calls at any point. All 204 stored answers were **re-scored**; **144 of them were read in
full** (the 24 affected questions x 2 arms x 3 runs). The remaining **60 were not read** — those 10
questions receive byte-identical excerpts on both arms, so the candidate is the same system on the
same input. That is the coverage, stated the same way everywhere in this report.
