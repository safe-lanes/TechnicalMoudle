# Open issues register — SAIL AI assistant candidate (as at 19-Sep-2026)
...
| A2 | **Audit History routing → a permission claim from the wrong feature.** "Who may comment in the Review section of an inspection history record" is routed to Technical; the Audit History manual is never supplied; the answer is built from the PMS *work-order* review section and turns a document audience label into a permission rule. Correct by coincidence. | `hist-1` r1–r3, the only unsupported claims in 1,245 → `citation-review-verdicts-s7.md` §C | **Open.** Two second-opinion routing variants were measured and neither worked (`ASSISTANT_SECOND_OPINION_GAP=0`). No fix attempted since. |
...
| A4 | `prep-3` and `sms-office-1` chunks are indexed but not retrieved. | S39 part 1 review | Open, unchanged. |
...
| C4 | The claim review read every claim below 0.50 coverage; **264 partial claims (0.50–0.79) were not individually read.** | Open, stated as a limit of the review. |
