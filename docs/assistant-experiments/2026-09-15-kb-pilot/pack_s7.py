...
ARMS = ["F0", "F1"]
CAP = {a: os.path.join(HERE, f"s7-{a.lower()}-capture.jsonl") for a in ARMS}
...
def main():
    ...
    import score8 as score7
    for a in ARMS:
        if os.path.exists(CAP[a]):
            score7.load_captures(a, CAP[a])
    rows = defaultdict(lambda: defaultdict(lambda: defaultdict(dict)))
    for name in ("answers", "generated", "wo", "manuals", "fresh"):
        p = os.path.join(HERE, f"s7-{name}-dump.jsonl")
    ...
    out = open(os.path.join(HERE, "s7-evidence-pack.txt"), "w", encoding="utf-8")
    out.write("FULL ANSWERS AND CAPTURED INPUTS — S6 paired run\n"
              "F0 = pre-change v5 candidate on kb-pilot-c 916 · F1 = identical, on kb-pilot-e 921 (restructured section + reconciled KB files)\n"
              "Built from the stored dumps and captures only; no service or model calls.\n")
    n = 0
    for suite in rows:
        for case in sorted(rows[suite]):
            v = {a: [score7.verdict(r)["pass"] for _, r in sorted(rows[suite][case].get(a, {}).items())] for a in ARMS}
            differs = v[ARMS[0]] != v[ARMS[1]]
            if not differs and case not in extra:
                continue
            n += 1
            rule, question = rule_and_question(suite, case)
            out.write("\n" + "#" * 118 + f"\n{suite.upper()} — CASE {case}\nQUESTION: {question}\nJUDGING RULE: {rule}\n"
                      f"PER-RUN (current judges): {ARMS[0]} {v[ARMS[0]]}  {ARMS[1]} {v[ARMS[1]]}\n")
            for arm in ARMS:
                for run, r in sorted(rows[suite][case].get(arm, {}).items()):
                    resp = r["response"]
                    ok = score7.verdict(r)
                    out.write("\n" + "=" * 110 + f"\n[{arm}] run {run} — verdict {ok['pass']} — {ok['detail']}\n")
                    ...
                    out.write(f"\nFULL ANSWER:\n{resp.get('response')}\n")
    out.close()
    print(f"wrote s7-evidence-pack.txt: {n} cases, {os.path.getsize(os.path.join(HERE, 's7-evidence-pack.txt')) // 1024} KB")
