...
> route requires the same vessel switch and returns the same message when it is off. Only the unplanned route is
> exempt from the switch. [code: jobService.generateWorkOrder → isOfficeWoGenerationEnabled, the same check
...
> per-job 'Generate WO' — and only the unplanned route is exempt from it.
