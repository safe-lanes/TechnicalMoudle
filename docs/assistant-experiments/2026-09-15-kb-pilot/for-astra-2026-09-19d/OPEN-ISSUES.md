...
| A6 | **Marker or related form turned into a rule** (same shape as A2b). `certsurveys-2`: a "Mandatory field" marker becomes "the record cannot be saved". `fn-2`: "the same steps are documented for a Lesson Learnt" when "Lesson Learnt" is absent from the supplied text. Both on F0. | `review-ledger-s7.md` | **Open.** Two new unsupported claims found this round. |
| A2b | **Permission inferred from a document's audience label.** The `hist-1` answers turn "Audience: Office / Sail Admin" into "only the office may add comments". An answer behaviour, not routing — not covered by the routing proposal. | `review-ledger-s7.md`, verdict `unsupported` | **Open.** No prompt rule proposed yet. |
