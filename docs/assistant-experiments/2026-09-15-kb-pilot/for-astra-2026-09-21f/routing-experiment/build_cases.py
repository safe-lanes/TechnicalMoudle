...
# ── expected-module corrections (reviewer, 21-Sep-2026) ────────────────────────────────────────────
# The first build took each case's `module` field as the expected answering module. For the two
# context cases in the fresh suite that field is the ORIGINATING widget module, not the expected
# answer — the opposite of what I recorded. Both corrections are written out with the suite's own
# words as the authority, rather than being applied silently; neither is inferred from a run.
EXPECTED_MODULE_OVERRIDE = {
    # fresh_cases.json score_by: "the module chosen must be Safety (the originating module sent by the
    # widget is technical and must not drag the question there)". `module: technical` is the CONTEXT.
    "fresh/fresh-ctx-1": ("safety", "suite score_by names Safety as the required module; the "
                                    "`module` field is the originating widget context"),
    # fresh_cases.json score_by: "ACCEPTABLE: route to Safety and state honestly that the Safety
    # documentation does not cover adding components". The expected FILE is the Technical manual, so
    # deriving the module from the file would also be wrong here — the agreed policy is to respect the
    # explicit module named in the question and explain the mismatch.
    "fresh/fresh-ctx-conflict-1": ("safety", "agreed policy: honour the module the question names and "
                                             "explain the mismatch; the Technical expected_file is the "
                                             "subject matter, not the required routing target"),
}
...
def pages_of(case: dict) -> list[int]:
    """The pages the case expects, so a supplied excerpt can be checked at PAGE level and not merely
    by filename. `pages` is stored as a string like "[5]" or "[5, 6]", and is absent on some cases."""
    raw = case.get("pages")
    if raw in (None, "None", ""):
        return []
    try:
    ...
    if isinstance(val, int):
        return [val]
    return [int(p) for p in val] if isinstance(val, (list, tuple)) else []
...
def build() -> list[dict]:
    ...
    for case in json.loads((IND / "manual_cases.json").read_text(encoding="utf-8")):
        ...
        out.append({"id": f"manuals/{case['id']}", "suite": "manuals", "question": case["question"],
                    ...
                    "expected_pages": pages_of(case), "expected_section": case.get("section")})
    ...
    for c in out:
        fix = EXPECTED_MODULE_OVERRIDE.get(c["id"])
        if fix:
            c["expected_module_original"] = c["expected_module"]
            c["expected_module"] = fix[0]
            c["expected_module_reason"] = fix[1]
    return out
