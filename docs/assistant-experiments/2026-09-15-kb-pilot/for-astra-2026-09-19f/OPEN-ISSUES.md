...
| A2 | **Audit History routing → a wrong-feature answer.** The inspection-history question is routed to Technical; the Audit History manual is never supplied; the answer is built from the PMS *work-order* review section. Correct by coincidence. | `hist-1` r1–r3 | **Open. Diagnosed 19-Sep, root cause located at MODULE SELECTION** (`ROUTING-DIAGNOSIS-where-the-passage-is-lost.md`): the passage IS indexed (`audit`, p.16); no Audit chunk reaches the distance floor; the question names no module; and the lexical channel — which ranks the correct passage **2nd corpus-wide** on 'review section office users' — runs **after** routing and is **scoped to the already-chosen module**, so it cannot correct a module error (`chat.py:103-108`). The question's own phrase 'inspection history' appears **0 times** in the corpus. Earlier claim of a *proven root cause in title matching* is **WITHDRAWN**. Nothing implemented. |
...
| # | issue | status |
|---|---|---|
| C7 | **R1 and R2 are LATENT, measured.** R1: no ambiguous title term exists on the current corpus. R2: of 42 stored questions that name a module, **0** were answered from a different module. Both are real code defects; **neither causes any observed misroute** and neither would fix `hist-1`. | Open, low priority. |
...
| C4 | **Review coverage.** Selected risk claims: **F1 123/123, F0 112/112 — complete.** 281 verdicts recorded (151 F1 + 130 F0). **Still unread and NOT verified: 2,186 occurrences** (1,203 machine-UNRESOLVED + 983 machine-`auto-supported`). **14 reviewed claims remain `unresolved`.** | **Open** — the wider review is incomplete. |
