# Review ledger — S7 paired run (v4, 19-Sep)

Stable id `suite/case/arm/rRUN/cNN`. **READ** verdicts are mine, from reading the claim against the
supplied text, each with document/section/page. **MACHINE** labels are deterministic rules;
`auto-supported` has **not** been read and is never reported as verified.

## Coverage — stated before any result

- **F1**: population 1255 · selected risk claims 123, **123 read, 0 unread** · total read **151**
- **F0**: population 1212 · selected risk claims 112, **112 read, 0 unread** · total read **130**
- **Both arms**: 281 verdicts recorded of 2467 occurrences.

**The wider review is INCOMPLETE.** 2186 occurrences are unread and NOT verified (1203 machine-UNRESOLVED + 983 machine-auto-supported). A further **14 reviewed claims remain `unresolved`** — read, but the evidence does not decide them. Open item C4.
...
|---|---|
| supported | 234 |
| supported-with-qualification | 27 |
| unresolved | 14 |
| unsupported | 5 |
| contradicted | 1 |

## Verdicts, grouped by finding


### CONTRADICTED — 1 occurrence(s), arm(s) F1
- **claim:** The draft code-derived guidance says the Office switch and Sail Admin restriction apply only to the relevant planned-generation actions, not to per-job Generate WO or unplanned work orders.
...
### UNSUPPORTED — 5 occurrence(s), arm(s) F0,F1
- **claim:** Only the Office / Sail Admin is allowed to add comments in the Review section.
- **ids:** `manuals/hist-1/F1/r1/c01`, `manuals/hist-1/F1/r2/c01`, `manuals/hist-1/F1/r3/c01`, `manuals/hist-1/F0/r2/c01`, `manuals/hist-1/F0/r3/c02`
- **evidence:** The expected source (Audit - History Manual_R1 p.16, 'The Review section is intended for office users only') was never supplied; the routing sent the question to Technical. The answer describes the PMS WORK-ORDER review procedure instead.
- **note:** CORRECTED FRAMING: I previously wrote that the permission was 'inferred from an audience label'. That overstates what the capture shows — the supplied text also says explicitly 'The user interface shown in Ref. Figures 12-15 are applicable to the Office side.' The Office attribution is therefore locally supported. The defect is that it is the WRONG FEATURE: work-order review, not inspection-history review.

### UNRESOLVED — 4 occurrence(s), arm(s) F0,F1
- **claim:** This restriction is stated for both vessel and office users.
...
### UNRESOLVED — 2 occurrence(s), arm(s) F1
- **claim:** Both use the new-document change-request process:
...
### UNRESOLVED — 1 occurrence(s), arm(s) F1
- **claim:** Office / Sail Admin
...
### UNRESOLVED — 1 occurrence(s), arm(s) F1
- **claim:** In the Office Certificates screen, you can:
...
### UNRESOLVED — 1 occurrence(s), arm(s) F1
- **claim:** <NNN> is the running sequence, which continues across both old and new numbering formats.
...
### UNRESOLVED — 1 occurrence(s), arm(s) F1
- **claim:** This procedure is also documented for both Ship and Office.
...
### UNRESOLVED — 1 occurrence(s), arm(s) F0
- **claim:** Existing older work orders retain their original numbers, and the sequence continues across both formats.
...
### UNRESOLVED — 1 occurrence(s), arm(s) F0
- **claim:** This applies in both Office and Ship documentation.
...
### UNRESOLVED — 1 occurrence(s), arm(s) F0
- **claim:** This restriction applies in both the vessel-specific and Office approval flows.
...
### UNRESOLVED — 1 occurrence(s), arm(s) F0
- **claim:** On the PMS Dashboard (Office), you can filter by:
- **ids:** `manuals/pmsoffice-5/F0/r3/c01`
- **evidence:** A list lead-in; the filter names are in the items beneath it.
- **note:** retained for visibility

### SUPPORTED-WITH-QUALIFICATION — 10 occurrence(s), arm(s) F0,F1
- **claim:** Therefore, the vessel-side Reports filters are not the same set as the Office PMS Dashboard filters.
- **ids:** `manuals/pmsoffice-5/F1/r1/c15`, `manuals/pmsoffice-5/F1/r1/c16`, `manuals/pmsoffice-5/F1/r2/c06`, `manuals/pmsoffice-5/F1/r2/c11`, `manuals/pmsoffice-5/F1/r3/c06`, `manuals/pmsoffice-5/F1/r3/c11`, `manuals/pmsoffice-5/F0/r1/c02`, `manuals/pmsoffice-5/F0/r1/c12` … +2 more
- **evidence:** OFFICE Dashboard filter 1.1.3.2 (p.9) and VESSEL Reports filter 1.1.9.3 (p.46) are both supplied, and the two filter lists differ as the answer states.
- **note:** VALID as a comparison of two DIFFERENT screens, which is what the question asks. NOT valid as a claim about equivalent screens: the two passages differ in BOTH screen and environment, so nothing here establishes that the same screen differs between Office and Vessel. Applies to both arms (earlier versions qualified F0 only).

### SUPPORTED-WITH-QUALIFICATION — 7 occurrence(s), arm(s) F0,F1
- **claim:** An office user can delete a Fleet Sharing record only while it is in Draft status.
...
### SUPPORTED-WITH-QUALIFICATION — 4 occurrence(s), arm(s) F0,F1
- **claim:** For an authorized Office user:
...
### SUPPORTED-WITH-QUALIFICATION — 2 occurrence(s), arm(s) F0,F1
- **claim:** The separate “+ New Entry” button creates a training need record, not a Training Matrix record.
...
### SUPPORTED-WITH-QUALIFICATION — 2 occurrence(s), arm(s) F0
- **claim:** A separate Training Needs process uses “+ New Entry”, followed by “Save”, but that creates a training need rather than a Training Matrix record.
...
### SUPPORTED-WITH-QUALIFICATION — 1 occurrence(s), arm(s) F1
- **claim:** For an Office user, add an action item as follows:
...
### SUPPORTED-WITH-QUALIFICATION — 1 occurrence(s), arm(s) F0
- **claim:** For the Stores sub-module, the filtering steps differ slightly between Office and Ship:
...
### SUPPORTED — 12 occurrence(s), arm(s) F0,F1
- **claim:** These are the same steps as Spares › How To Apply Filter, section 1.1.7.7, page 47.
...
### SUPPORTED — 10 occurrence(s), arm(s) F0,F1
- **claim:** The three-way list, including automatic generation and the Office switch/role conditions, comes from draft code-derived guidance based partly on application code revision cf5241ad6.
...
### SUPPORTED — 10 occurrence(s), arm(s) F0,F1
- **claim:** It is not a completely separate procedure for each section, but the available filter options vary.
...
### SUPPORTED — 8 occurrence(s), arm(s) F0,F1
- **claim:** If the approver needs a different Approval Level, edit the existing approver record instead of creating a new one.
...
### SUPPORTED — 7 occurrence(s), arm(s) F0,F1
- **claim:** To create a COC defect record, follow the same steps as “How To Create A New Defect” under Defects Logs, section 1.1.4.3, page 15:
...
### SUPPORTED — 7 occurrence(s), arm(s) F0,F1
- **claim:** For Rejected and Recruited candidates, the steps are the same as In Progress, section 1.2.1.2, page 13.
...
### SUPPORTED — 7 occurrence(s), arm(s) F0,F1
- **claim:** As a vessel user, you can only view Generic Risk Assessments in the Generic RA Library; you cannot edit them there.
...
### SUPPORTED — 6 occurrence(s), arm(s) F0,F1
- **claim:** These steps are the same as In Progress, section 1.2.1.5, page 19.
...
### SUPPORTED — 6 occurrence(s), arm(s) F0,F1
- **claim:** Duplicate checking applies only when a Job Code is supplied together with a Component Code for the selected vessel.
...
### SUPPORTED — 6 occurrence(s), arm(s) F0,F1
- **claim:** Alternatively, the Sail Admin can contact Support to re-provision or update the ship.
...
### SUPPORTED — 6 occurrence(s), arm(s) F0,F1
- **claim:** Multiple attachments may be added to one question.
...
### SUPPORTED — 6 occurrence(s), arm(s) F0,F1
- **claim:** The combined size of files selected in one upload must not exceed 20 MB, and attachments added between consecutive saves or synchronizations should not exceed 150 MB.
...
### SUPPORTED — 6 occurrence(s), arm(s) F0,F1
- **claim:** The hyperlinking example shows a link to the specific document reference “Doc 4.2.03.”
...
### SUPPORTED — 5 occurrence(s), arm(s) F0,F1
- **claim:** The manual states that a new component can also be added by clicking the “+ Add Component” button.
...
### SUPPORTED — 5 occurrence(s), arm(s) F0,F1
- **claim:** The manual explicitly says to follow the same attachment-and-save procedure for Positive Observations and LAE finding details.
...
### SUPPORTED — 5 occurrence(s), arm(s) F0,F1
- **claim:** The Crew Pool form uses the same steps as the “How to Create a New Crew Record” section (p.
...
### SUPPORTED — 5 occurrence(s), arm(s) F0,F1
- **claim:** This applies to both Ship and Office procedures.
...
### SUPPORTED — 5 occurrence(s), arm(s) F0
- **claim:** They therefore do not document a separate, complete Stores procedure.
...
### SUPPORTED — 4 occurrence(s), arm(s) F0,F1
- **claim:** The second way is to use the Date / Time / Description fields:
...
### SUPPORTED — 4 occurrence(s), arm(s) F0,F1
- **claim:** Parts B and C: The Add SM form does not provide separate instructions here.
...
### SUPPORTED — 4 occurrence(s), arm(s) F0,F1
- **claim:** Follow the instructions in the Monthly Safety Meeting form.
...
### SUPPORTED — 4 occurrence(s), arm(s) F0,F1
- **claim:** These steps are the same as “How To Export All Defect Report,” section 1.1.4.5, page 17.
...
### SUPPORTED — 4 occurrence(s), arm(s) F1
- **claim:** Normal sign-in and vessel access also apply, and a job with an active work order cannot receive another one.
...
### SUPPORTED — 3 occurrence(s), arm(s) F0,F1
- **claim:** This applies to both the Office and Vessel/Ship manuals.
...
### SUPPORTED — 3 occurrence(s), arm(s) F0,F1
- **claim:** For the Office instance, the draft code-derived guidance says the vessel’s office work-order generation switch must be enabled; for Ship, no switch is required.
...
### SUPPORTED — 3 occurrence(s), arm(s) F1
- **claim:** The excerpts do not support that a Head of Department can do everything an office user can do.
...
### SUPPORTED — 3 occurrence(s), arm(s) F0,F1
- **claim:** Auto-sync, when enabled on the ship, is the other supported way to continue synchronization.
...
### SUPPORTED — 3 occurrence(s), arm(s) F0,F1
- **claim:** Ship users cannot modify it.
...
### SUPPORTED — 3 occurrence(s), arm(s) F1
- **claim:** For a Positive Observation or LAE finding, use the same Upload, Save, and Inspection steps for the corresponding finding details.
...
### SUPPORTED — 3 occurrence(s), arm(s) F1
- **claim:** However, the provided survey excerpt does not reproduce the actual survey steps, so it is not possible to confirm from this documentation whether the process is different from certificates.
...
### SUPPORTED — 3 occurrence(s), arm(s) F1
- **claim:** No user action is required.
...
### SUPPORTED — 3 occurrence(s), arm(s) F0
- **claim:** The documentation does not say that a Head of Department can do everything an Office user can do.
- **ids:** `generated/7/F0/r2/c02`, `generated/7/F0/r3/c02`, `generated/7/F0/r3/c07`
- **evidence:** Roles & Permissions 1.1.12.6: 'Head of Dept does not approve postponement requests when no approval steps are configured — that is reserved for office roles.'

### SUPPORTED — 3 occurrence(s), arm(s) F0
- **claim:** The excerpts do not state that the process is different from certificates.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0,F1
- **claim:** These categories are shown as tabs in the hazard-selection window for both Office and Vessel risk assessments.
- **ids:** `answers/5/F1/r3/c08`, `answers/5/F0/r1/c08`
- **evidence:** RA Office (p.13) and RA Vessel 8 (p.12) both supplied.

### SUPPORTED — 2 occurrence(s), arm(s) F1
- **claim:** The draft code-derived guidance says this path has no Sail Admin role requirement; the published manuals present the action without these conditions.
...
### SUPPORTED — 2 occurrence(s), arm(s) F1
- **claim:** No specific role is required, but normal sign-in and vessel access apply.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0,F1
- **claim:** The manual explicitly says to follow the same procedure for adding Positive Observations and LAE finding details.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0,F1
- **claim:** Office: It opens the selected MoC record so you can view, edit, or export it.
...
### SUPPORTED — 2 occurrence(s), arm(s) F1
- **claim:** The same process can be used for the selected category—Stores, Lubes, Chemicals, or Others—and for the selected tab—Inventory, Location, or History.
...
### SUPPORTED — 2 occurrence(s), arm(s) F1
- **claim:** The manuals describe this process for both Vessel and Office use; the Office manual provides the detailed transaction-entry and save steps.
...
### SUPPORTED — 2 occurrence(s), arm(s) F1
- **claim:** The provided documentation does not fully establish a separate, complete Store procedure beyond its reference to Spares.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0,F1
- **claim:** In the office: planned work orders are not generated automatically.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** The June manuals describe the action as unconditional, except that they also require no active duplicate work order; this differs from the code-derived guidance.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** The vessel switch is not required.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** The same recording steps are also documented for a Lesson Learnt.
- **ids:** `manuals/fn-2/F0/r1/c08`, `manuals/fn-2/F0/r3/c08`
- **evidence:** The Lesson Learnt User Manual_Vessel_R0 (p.7) Section 2 Discussion Record WAS supplied as excerpt 2 and is cited. PROCEDURES COMPARED (the reviewer's caveat): the two supplied passages are word-for-word the same — 'In Section 2 - Discussion Record, record the details of the onboard discussion / Select the Notification Discussed on board checkbox / Enter the Date of discussion / Enter the participant details / submitted by clicking Submit'. 'Same steps' is therefore verified, not merely inferred from presence.
- **note:** RETRACTION: I recorded this as UNSUPPORTED last round on the grounds that 'Lesson Learnt' appears nowhere in the supplied text. It appears in the excerpt HEADER; my search covered only the body. Third tooling false finding in this workstream, after the em-dash header parser and the negated-XREF rule.

### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** Do not create a new approver record for the same Position/Rank.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** If mandatory fields are blank, the record cannot be saved.
- **ids:** `manuals/certsurveys-2/F0/r1/c08`, `manuals/certsurveys-2/F0/r2/c06`
- **evidence:** Audit/Cert. & Surveys supplied text, verbatim: 'Mandatory fields (*) must be completed before the record can be saved.' The answer's 'if mandatory fields are blank, the record cannot be saved' is the contrapositive of that sentence.
- **note:** RETRACTION: I recorded this as UNSUPPORTED last round. The verdict was wrong — I searched for 'cannot be saved' / 'not be saved' and missed the manual's positive phrasing. Absence of a phrase is not absence of support, exactly as the reviewer said.

### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** This applies in both Office and Ship instances.
...
### SUPPORTED — 2 occurrence(s), arm(s) F0
- **claim:** It creates a work order when the job is active, attached to a component, has a frequency and next due date, and has no active work order already.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** It does not provide separate steps for that button.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** Recruited — uses the same steps as In Progress, section 1.2.1.2, page 13:
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** If the switch is OFF, this office generation path cannot be used; the excerpt does not specify a separate error message.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** No specific role or office-generation switch is required; normal sign-in and vessel access still apply.
- **ids:** `generated/6/F1/r1/c15`
- **evidence:** Read in context: the sentence is line 17 of the answer, under the heading '3. Create an unplanned work order'. The same answer states the per-job switch requirement correctly at line 13.

### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** The role and switch behavior above comes from draft, code-derived guidance rather than a published manual.
- **ids:** `generated/6/F1/r1/c19`
- **evidence:** The KB files carry the provenance note verbatim in the supplied text.

### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** It does not require the office generation switch.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** The same non-decrease rule also applies to direct ship/office entries and synchronization; synchronization reconciles readings by date, with the latest date winning and a same-date tie going to the ship.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** Section 4.2.2 is not included in the provided excerpts, and the manual does not state that its approval steps are the same as Section 4.1.2.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** This requires no active WO for the job.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** The June manuals document the unplanned-work-order procedure and state that they cover only the on-demand and unplanned ways; the provided published excerpts show the unplanned procedure but do not provide the separate per-job procedure.
...
### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** The draft guidance says the Sail Admin check was inspected in application code and was not measured against a running installation.
- **ids:** `wo/wo-phr-04/F1/r3/c06`
- **evidence:** KB text: 'Read from the source code; not measured against a running installation.'

### SUPPORTED — 1 occurrence(s), arm(s) F1
- **claim:** The published manual describes the same navigation and reason selection, and warns that duplicate active work orders are not allowed; it does not state the office switch condition.
...
### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** This applies in both the Office and Vessel/Ship manuals.
- **ids:** `answers/10/F0/r1/c02`
- **evidence:** MOC Office 6 (p.11) and MOC Vessel 5 (p.7) both supplied.

### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** The role and switch details are from draft, code-derived guidance verified against application revision cf5241ad6; the operational update also states the Sail Admin and switch requirements.
- **ids:** `generated/6/F0/r2/c17`
- **evidence:** The KB provenance note names revision cf5241ad6 verbatim in the supplied text.

### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** A separate supported process is importing a JSON file: inspection and observation details—including Positive and LAE findings—are populated automatically, while any additional supporting attachments must be uploaded manually.
- **ids:** `manuals/hist-3/F0/r3/c13`
- **evidence:** Audit History Manual (p.13), verbatim: 'Observation details, including Positive and LAE findings, are automatically populated from the uploaded JSON file.'

### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** Yes, its function differs between the manuals.
- **ids:** `manuals/moc-office-1/F0/r2/c03`
- **evidence:** MOC Office 3 (p.7) and MOC Vessel 2 (p.5) action tables both supplied.

### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** So, the process is shared, but each section may present different filtering options.
...
### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** As a vessel user, you can only look at Generic Risk Assessments in the Generic RA Library; editing is not available there.
...
### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** These categories are available in the Store sub-module for both Office and Vessel users.
- **ids:** `manuals/pmsvessel-1/F0/r3/c06`
- **evidence:** PMS Office 1.1.8.1 and PMS Vessel 1.1.8.1 — the same section from both environments — are both supplied.

### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** The supplied vessel manual documents only the first two Spares steps; it does not provide the later transaction-mode, quantity, or save steps.
- **ids:** `manuals/pmsvessel-3/F0/r1/c15`
- **evidence:** Accurate limitation about what the Vessel passage contains.

### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** The Office manual includes vessel selection, while the Vessel Specific manual does not include that step.
...
### SUPPORTED — 1 occurrence(s), arm(s) F0
- **claim:** The draft code-derived guidance is broader than the published June manuals: the manuals describe the on-demand and unplanned procedures, while the draft guidance also describes automatic ship generation and office Generate Now, and adds the office co
