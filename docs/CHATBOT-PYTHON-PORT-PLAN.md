...
**S.7 — Follow-up 1: the generic "How do I create a work order?" (14-Sep-2026; candidate only,
live unchanged, deployment needs owner approval).**
*Finding:* the deployed assistant answers only the unplanned-work-order procedure — an
unsupported assumption; the question does not say "unplanned".
*What the manuals establish (PROVEN, saved parses of the official PMS manuals):* Scheduled /
Due / Overdue tabs = "jobs that have been planned and have a future due date" (Office p28
§1.1.5.1); Components Part C lists each job's frequency and last/next due dates and offers
'Generate WO' with a reason (Office p18 / Vessel p16); '+ Unplanned W.O' with Part A save and Part
B submit (Office p29 §1.1.5.2 / Vessel p24). No manual section explains how planned work orders
are produced. Section list checked: PMS has no Jobs sub-module section (1.1.1–1.1.11).
*What the Technical code establishes (PROVEN on this branch; the seven work-order files are
byte-identical to `origin/replit_dev` cf5241ad6 = dev deployment; on the fork line
`origin/feature/approval-engine-phase2` 037c7e98f only `work-orders/routes.ts` differs, in the
postponement-approval guards, not in creation):* planned work orders are generated by the ship's
daily job-due scan (`server/services/jobDueScanner.ts`, ship-only); office generation only via
'Generate Now' = Sail Admin AND the vessel's office-generation switch
(`workOrderGenerationGate.ts:44,141-161`); the per-job 'Generate WO' (`Components.tsx:916` →
`POST /jobs/:id/generate-wo`, `jobService.ts:570-590`) is on the office side gated by the vessel
switch only (no Sail-Admin-only check on that path), reasons Planning / Breakdown / Other,
duplicate refused; unplanned = `POST /work-orders` from the unplanned form.
*Mismatches:* (1) the June manuals show 'Generate WO' unconditionally; on the office side it needs
the vessel switch. (2) The manuals never say planned work orders are automatic; only the
code-derived document does. (3) UI/permission display: 'Generate Now' is shown to Sail Admin,
Client Admin and Head of Dept but the server allows Sail Admin only — standalone note for the domain
team: `docs/assistant-experiments/2026-09-14-repairs/note-generate-now-ui-mismatch.md`.
*Fix (candidate):* new code-derived section "1.1.14.13 How To Create A Work Order" in the Recent
Updates document (R3.2, `generated-docs/build_r3.py`, PROVENANCE +4 rows with file:line), keeping
the three paths distinct with their verified conditions and marking where it supersedes the June
manuals; new index set `repaired-r32` built from the isolated candidate corpus (914 chunks; only
that document re-parsed, everything else reused); the live set `repaired` untouched.
*Retrieval, measured not assumed:* with the first heading ("…— The Three Ways (Planned, Per-Job
'Generate WO', Unplanned)") the new chunks ranked 6 and 8 for the generic question — outside the
5 excerpts the answer receives (`answer_chunks`), so the paragraph alone did NOT fix it. A shorter
heading ("How To Create A Work Order") with a compact three-way opening sentence moved the chunk
to rank 1 (0.7919 vs 0.7927 for the manual's unplanned section), rank 1 for the lower-case form,
rank 4 for "How to create work order in PMS?", rank 1 for the explicit planned-work-order question.
No retrieval-logic change was needed; `answer_chunks` stays 5.
*New regression suite (separate):* `indexer/acceptance_wo.py` (`WO_SUITE_VERSION 2026-09-14.1`) —
wo-generic-01 (the generic question: must name '+ Unplanned W.O', 'Generate WO' and the Sail Admin
condition, must mention automatic/scheduled generation, cite a Technical source) and
wo-generic-02 (explicit planned question). Frozen suites untouched. Results: §S.7 run E below.
*Results (candidate `repaired-r32` on experiment instance 8018, image `prompt-v2`, prompt v2;
baseline = live's `repaired` on experiment instance 8016, same image/prompt; 3-run majority;
`item1-run-e.txt`, `item1-run-f.txt`, `item1-run-g.txt` + dumps):*

| suite | baseline (repaired) | candidate (repaired-r32) |
|---|---|---|
| retrieval, 18 queries (run E / F) | 18/18 | **17/18** — "how do I create a work order": module ✓, top manual ✗ because the code-derived Recent Updates note now ranks first (0.7937 vs the manual's 0.8084); the suite's expectation for that query is "top source = PMS User Manual" |
| frozen 12-case (.3), run E | 11/12 | 11/12, no regression, no gains |
| corrected claims (.2), run E | 12/14 | 12/14 (04 = known citation limitation; 13 = 1/3 on BOTH — the judge's phrase "back-dated" vs the model's paraphrase "reading date earlier than…", run-to-run, not caused by this change) |
| work-order suite .1, run E | 0/2 | 2/2 — **NOT a true pass**: the .1 judge accepted "Sail Admin" from the manual's FILE NAME in the Source line while the body stated no office condition (3/3 runs) |
| work-order suite .2 (body-only judge), run F, after restructuring the section | — | wo-generic-01 **2/3**, wo-generic-02 3/3 |
| frozen 12-case (.3) + corrected 14 (.2), run G on the FINAL restructured build | (run D: 11/12 · 13/14) | **11/12 · 13/14** — same cases as run D (05 clarify; 04 citation), no regression |

*Run F answers, read in full:* runs 1–2 give all three ways with the office conditions for
planned generation ("only available for a Sail Admin", "ensure the vessel's switch is on") and the
ship rule; run 3 keeps the switch condition but drops the Sail Admin role; runs 1–2 omit the office
switch condition for the per-job 'Generate WO' path. So the content is now retrievable and the
answer is no longer "unplanned only", but the model still condenses conditions inconsistently.
*Reported, not redefined:* (1) the retrieval suite's 17/18 is a regression against that suite's
expectation; the source-justified correction would be to accept the code-derived Recent Updates
note as the expected top source for that query (the note cites the manual sections) — proposed,
NOT applied; (2) the judge change .1 → .2 is a correction of my own judge, reported; (3) the
"How to create work order in PMS?" phrasing ranks the note 6th after the restructuring (4th
before) — the fix is sensitive to phrasing.
*Proposal for the residual (needs owner approval — it is a prompt change that touches every
answer):* one rule in the docs-path answer prompt: "when an excerpt attaches a condition to a
step (who may do it, a switch that must be on, ship vs office), state that condition with the
step; never present a conditional action as unconditional". To be measured on the 18 + 12 + 14 +
work-order suites before any deployment. Alternative without a prompt change: accept the current
2/3 as the candidate's result.
