...
var StatsAccumulator = class {
  ...
  logAccumulatedStats() {
    ...
    import_ag_charts_core23.Logger.log(`\u{1F4CA} Stats (${timeSinceLastLog.toFixed(0)}s, ${count} renders): ${parts.join(" ")}`);
    this.stats.clear();
    this.lastLogTime = Date.now();
  }
  destroy() {
...
var _Text = class _Text extends Shape {
  ...
  onTextChange() {
    ...
    if ((0, import_ag_charts_core26.isArray)(this.text)) {
      ...
      this.richText.append(
        this.text.flatMap((s) => (0, import_ag_charts_core26.isString)(s.text) ? s.text.split(import_ag_charts_core26.LineSplitter) : String(s.text)).filter(Boolean).map(() => new _Text({ trimText: false }))
      );
    } else {
  ...
  static measureBBox(text, x, y, options) {
    if ((0, import_ag_charts_core26.isArray)(text)) {
      ...
      const { width, height, lineMetrics } = (0, import_ag_charts_core26.measureTextSegments)(text, font);
      ...
      return _Text.computeBBox(text?.split(import_ag_charts_core26.LineSplitter), x, y, options);
    }
  }
  ...
  render(renderCtx) {
    ...
    if ((0, import_ag_charts_core26.isArray)(this.text) && this.richText) {
      this.generateTextMap();
      const richTextBBox = this.richText.getBBox();
      const { width, height, lineMetrics } = (0, import_ag_charts_core26.measureTextSegments)(this.text, this);
      let translateX = 0;
      ...
      switch (this.textBaseline) {
        ...
        case "middle":
          translateY -= lineMetrics.length === 1 ? lineMetrics[0].ascent + lineMetrics[0].segments.reduce(
            (offsetY, segment) => Math.min(offsetY, (0, import_ag_charts_core26.cachedTextMeasurer)(segment).baselineDistance("middle")),
            0
          ) : height / 2;
          break;
        case "bottom":
...
var Scene = class extends import_ag_charts_core36.EventEmitter {
  ...
  render(opts) {
    ...
    const statsEnabled = Debug.check("scene:stats" /* SCENE_STATS */, "scene:stats:verbose" /* SCENE_STATS_VERBOSE */);
    if (statsEnabled) {
...
var RadialColumnShape = class extends Path {
  ...
  updateBeveledPath() {
    ...
    const getTriangleHypotenuse = (leg, otherLeg) => Math.hypot(leg, otherLeg);
