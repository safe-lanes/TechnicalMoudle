...
-   **PMS Dashboard**: Professional analytics workspace with tabbed layout (Overview, Departments, Equipment, Compliance, Financial). All 14 visualizations built exclusively with AG Charts React (`ag-charts-react`), featuring interactive legends, detailed tooltips, and responsive design. Charts derive data from real filtered work orders with no mock/random data. Includes KPI cards with sparklines, stacked bar charts, line/area charts, pie charts, scatter/bubble plots, bubble-based heatmaps, grouped bar charts (hierarchical cost views), waterfall charts, and multi-series financial trends. All visualizations respond to vessel and date range filters.
