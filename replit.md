...
- **Component Identity Restructure (cuuid Full Migration Complete)**: `components.cuuid` is `TEXT NOT NULL UNIQUE` — the canonical UUID identity column for all relational references. All 1,638 existing rows were populated with `gen_random_uuid()` via migration 0016. New components auto-generate `cuuid` via `crypto.randomUUID()` in all creation paths. The existing `components.id` (TEXT PK, format `COMP-{ts}-{rand}`) remains unchanged as the primary key but is no longer used for relational lookups. **15 child tables have DB-level FK constraints** (`component_id` → `components.cuuid`, ON DELETE RESTRICT, ON UPDATE CASCADE) via migrations 0018-0021. **Full server-side migration**: All ORM queries use `eq(components.cuuid, ...)` — zero remaining `eq(components.id, ...)` patterns. All child table inserts store `component.cuuid` (not `component.id`). All JOIN conditions use `components.cuuid`. All `inArray` lookups use `components.cuuid`. Refactored files: `postgresStorage.ts` (29+ locations), `routes.ts` (12 locations), `runningHoursRoutes.ts` (2 locations), `routes/bulk.ts` (23 locations), `tests/db/verification.ts`, all V2 repositories (components, running-hours, work-orders, jobs), all V2 services. Schema defined in both `shared/schema.ts` and all V2 schemas. `cuuid` is omitted from insert schemas (auto-generated).
- **Vessel Identity Restructure (Phase 4 Complete)**: `vessels.id` is now `SERIAL PRIMARY KEY` (integer auto-increment). `vessels.vuuid` is `TEXT NOT NULL UNIQUE` (the canonical UUID identity column). All 31 child tables' `vessel_id TEXT` columns reference `vessels(vuuid)` via FK constraints (migrations 0009-0013). Migration 0014 converted `id` from TEXT to SERIAL. **Server**: All ORM queries use `eq(vessels.vuuid, ...)`. `getVessels()` returns `{ id: number, vuuid: string, name, code }`. `createVessel()` auto-generates integer `id`, accepts `vuuid` for UUID. Sync-masters INSERT omits `id` (auto-generated), upserts on `vuuid`. **Frontend**: VesselContext/useVessels Vessel interface has `id: string` (mapped from vuuid) and `vuuid: string`. All 30+ page files using `vessel.id` work without changes since the UUID value is identical. Child tables' `vessel_id` columns remain TEXT referencing `vessels.vuuid` — no child table migration needed.
