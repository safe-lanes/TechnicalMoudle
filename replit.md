...
- **PMS Dashboard Analytics**: All dashboard charts use real database data (no hardcoded/fake data). Includes: Work Order Status Distribution donut, Outstanding Tasks donut with trend delta, 6-Month Maintenance Trend bar chart, Spares Stock Status donut, 6-Month Compliance Rate Trend bar chart, 6-Month Spares Movement Trend (consumption vs receiving) grouped bar chart. Running Hours card shows real RH-tracked component counts (master + inherited) and recent update activity. All analytics support "All Vessels" aggregation via parallel per-vessel API fetching with Promise.allSettled.
