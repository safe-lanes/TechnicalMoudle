...
- **Identity Restructuring (Components, Jobs, Vessels, Work Orders — All Complete)**: Implemented UUID-based canonical identifiers (`cuuid`, `juuid`, `vuuid`, `wouuid`) for Components, Jobs, Vessels, and Work Orders, alongside serial integer primary keys. All four entities have fully completed all 4 phases (UUID column, FK constraints, code refactoring, id TEXT→SERIAL conversion). Work Orders migration: `wouuid` column (migration 0028), FK constraints on 4 child tables (migrations 0029-0030), full code refactoring (Phase 3), and id TEXT→SERIAL conversion (migration 0031, Phase 4). All server/frontend code uses `wouuid` for identity lookups; `work_orders.id` is now SERIAL PRIMARY KEY auto-increment.
