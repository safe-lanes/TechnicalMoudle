...
- **Identity Restructuring (Components, Jobs, Vessels — Complete; Work Orders — Phase 3 of 4)**: Implemented UUID-based canonical identifiers (`cuuid`, `juuid`, `vuuid`, `wouuid`) for Components, Jobs, Vessels, and Work Orders, alongside serial integer primary keys. Components, Jobs, and Vessels have fully completed all 4 phases (UUID column, FK constraints, code refactoring, id TEXT→SERIAL conversion). Work Orders (`wouuid`) has completed Phases 1-3 (UUID column migration 0028, FK constraints on 4 child tables migrations 0029-0030, full code refactoring) — all server/frontend code now uses `wouuid` for identity lookups. `work_orders.id` is still TEXT PRIMARY KEY — Phase 4 (id conversion to SERIAL) is pending.
