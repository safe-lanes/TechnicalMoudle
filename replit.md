...
- **Component Identity Restructure (cuuid Column Added)**: `components.cuuid` is `TEXT NOT NULL UNIQUE` — a canonical UUID identity column for components. All 1,638 existing rows were populated with `gen_random_uuid()` via migration 0016. New components auto-generate `cuuid` via `crypto.randomUUID()` in all creation paths (postgresStorage, V2 repository, V2 bulk import). The existing `components.id` (TEXT PK, format `COMP-{ts}-{rand}`) remains unchanged as the primary key. The `cuuid` column is preparation for future child table FK migration (similar to the vessels `vuuid` pattern). Schema defined in both `shared/schema.ts` and `shared/v2/components/schema.ts`. `cuuid` is omitted from insert schemas (auto-generated).
