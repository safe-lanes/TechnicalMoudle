...
- **Chatbot Analytical Upgrade (Feb 2026)**: Enhanced from data-dump to intelligent analyst. System prompt enforces structured responses (Summary → Critical Insights → Priority Items → Recommendations). 5 new analytical tools: `get_maintenance_insights` (KPIs, compliance rate, overdue aging), `get_spare_coverage_analysis` (shortage risk, reorder urgency, stockout estimates), `get_workload_analysis` (backlog aging buckets, department distribution), `get_component_health_score` (risk scoring combining defects + overdue + criticality), `get_performance_trends` (completion rates, resolution times, trend direction). Existing tools enhanced with analytical summaries (priority breakdowns, aging, urgency scoring). Config: max_tokens=4000, maxIterations=8, temperature=0.4. Total: 20 function-calling tools.
