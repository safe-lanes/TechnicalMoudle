...
-   **Work Order Attachment Display**: All attachment sections in work order forms display icon-only rows (Paperclip + Eye + Trash2) without file names or file sizes. The View icon opens the document in a new browser tab via `window.open(dataUrl, '_blank')`. The Delete icon shows a confirmation dialog ("Are you sure you want to delete this attachment?"). This applies to all 4 attachment areas in `WorkOrderFormPage.tsx` (riskAssessment, safetyChecklist, operationalForm, other) and all 3 in `WorkOrderForm.tsx`.
