...
- **Component Identity Restructure (cuuid FK Phase Complete)**: `components.cuuid` is `TEXT NOT NULL UNIQUE` — a canonical UUID identity column for components. All 1,638 existing rows were populated with `gen_random_uuid()` via migration 0016. New components auto-generate `cuuid` via `crypto.randomUUID()` in all creation paths (postgresStorage, V2 repository, V2 bulk import). The existing `components.id` (TEXT PK, format `COMP-{ts}-{rand}`) remains unchanged as the primary key. **13 child tables now have DB-level FK constraints** (`component_id` → `components.cuuid`, ON DELETE RESTRICT, ON UPDATE CASCADE) via Drizzle auto-generated migrations 0018-0020: jobs (component_id NOT NULL + FK via migration 0020), job_component_links, spare_component_links, spares, spares_history, work_order_executions, running_hours_audit, component_documents, component_class_regulatory, component_maintenance_history, component_requisitions, component_running_hours_log, defects. Schema defined in both `shared/schema.ts` and all V2 schemas (components, spares, work-orders, running-hours, jobs). `cuuid` is omitted from insert schemas (auto-generated).
