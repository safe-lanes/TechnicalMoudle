...
- **Component Identity Restructure (Full Migration Complete)**: `components.id` is now `SERIAL PRIMARY KEY` (integer auto-increment). `components.cuuid` is `TEXT NOT NULL UNIQUE` — the canonical UUID identity column for all relational references. Migration 0022 converted `id` from TEXT (COMP-xxx format) to SERIAL, following the proven vessels migration 0014 pattern. **15 child tables have DB-level FK constraints** (`component_id` → `components.cuuid`, ON DELETE RESTRICT, ON UPDATE CASCADE) via migrations 0018-0021. **Server**: All ORM queries use `eq(components.cuuid, ...)` for lookups. All child table inserts store `component.cuuid`. All JOIN conditions use `components.cuuid`. `createComponent()` auto-generates integer `id` (omitted from insert). COMP-xxx ID generation removed from all 4 creation paths. Upload services map 'Component ID' spreadsheet column to `componentCode` (not `id`). **Frontend**: `component.id` is now `number` type. SelectItem values use `String(component.id)`. Schema defined in both `shared/schema.ts` and `shared/v2/components/schema.ts`. Both `id` and `cuuid` are omitted from insert schemas (auto-generated).
- **Jobs Identity Restructure (Complete)**: `jobs.id` is now `SERIAL PRIMARY KEY` (integer auto-increment). `jobs.juuid` is `TEXT NOT NULL UNIQUE` — the canonical UUID identity column for all relational references. Migration 0025 added the `juuid` column with safe backfill pattern (nullable → populate with gen_random_uuid() → NOT NULL → UNIQUE). Migration 0026 added **4 FK constraints**. Migration 0027 converted `id` from TEXT (JOB-xxx format) to SERIAL, following the proven vessels migration 0014 and components migration 0022 pattern. JOB-xxx/FJ-xxx ID generation removed from all 4 creation paths (createJob, bulkCreateJobs, bulkUpsertJobs, createFleetJob). `createJob()` auto-generates integer `id` (omitted from insert) from child tables to `jobs(juuid)`: work_orders, job_component_links, component_maintenance_history, fleet_job_vessel_mapping (all ON DELETE/UPDATE NO ACTION). **Server**: All ORM queries use `eq(jobs.juuid, ...)` for lookups. All child table inserts store `job.juuid` in `job_id` columns. All JOIN conditions use `jobs.juuid`. `createJob()` auto-generates `juuid` via `crypto.randomUUID()`. All job CRUD (getJob, updateJob, deleteJob, archiveJob, getFleetJob) query by `juuid`. Bulk upload (legacy `server/routes/bulk.ts` and V2 `bulkImportService.ts`) uses `juuid` for job-component links, change tracking, and auto-resolving. Services (jobDueScanner, workOrderService) use `job.juuid` for WO matching and linking. **V2 modules**: All repositories (jobRepository, workOrderRepository, bulkRepository, componentRepository) query by `juuid`. All services use `job.juuid` for identity. **Frontend**: All 7 client files use `job.juuid` for navigation URLs, React keys, data-testid attributes, and API calls. `job.id` is no longer used for identity/linking anywhere.
- **Vessel Identity Restructure (Complete)**: `vessels.id` is now `SERIAL PRIMARY KEY` (integer auto-increment). `vessels.vuuid` is `TEXT NOT NULL UNIQUE` (the canonical UUID identity column). **`vessels.code` must never be used for vessel identity/lookups** — only `vessels.vuuid` serves that purpose (`vessels.code` currently happens to contain the same UUID but is a separate business field). **All 43 child tables** now have `vessel_id TEXT` columns referencing `vessels(vuuid)` via FK constraints: 31 original tables (migrations 0009-0013) + 12 tables migrated from `vessel_code` (migration 0024). No `vessel_code` columns remain in the database. Migration 0014 converted `id` from TEXT to SERIAL. **Server**: All ORM queries use `eq(vessels.vuuid, ...)`. `getVessels()` returns `{ id: number, vuuid: string, name, code }`. `createVessel()` auto-generates integer `id`, accepts `vuuid` for UUID. Sync-masters INSERT omits `id` (auto-generated), upserts on `vuuid`. **Frontend**: VesselContext/useVessels Vessel interface has `id: string` (mapped from vuuid) and `vuuid: string`. All 30+ page files using `vessel.id` work without changes since the UUID value is identical. Child tables' `vessel_id` columns remain TEXT referencing `vessels.vuuid` — no child table migration needed.
