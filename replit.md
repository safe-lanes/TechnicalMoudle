...
=======
- **AI Chatbot Assistant**: Floating chat panel (bottom-right) powered by OpenAI GPT-4o with function calling. Features 15 tools for querying work orders, spares, components, jobs, stores/lubricants/chemicals inventory, defects (active/resolved/recurring), consumption pattern analysis, and maintenance calendar/scheduling. Uses lazy-initialized OpenAI client (requires OPENAI_API_KEY env var). Frontend uses react-markdown for safe rendering. Files: `server/services/chatbotService.ts`, `server/routes/chatbot.ts`, `client/src/hooks/useChat.ts`, `client/src/components/chat/`.
- **V2 Fleet Module Refactor Plan**: Comprehensive planning document (`attached_assets/V2-Fleet-Module-Refactor-Plan.md`) mapping all 85 fleet route handlers across `routes.ts` and `fleetAdmin.ts` to a V2 modular Repository/Service/Controller architecture. Covers 12+ entities (fleet components, jobs, spares, makers, master lists, 4 mapping types, registry, dashboard, master data, import history, copy vessel). Includes route inventory checklist, storage method reconciliation, business logic preservation rules (maker auto-code, job sanitization, copy vessel cascade, dashboard stats aggregation), dual fleet component path resolution, and phased migration plan with toggle-based runtime switching.
>>>>>>> 9cc061ada4ffef1ad8e89fbbecfe56088b8be6ff
