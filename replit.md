...
## Recent Changes
- **Work Order Completion Date Fix** (Jan 13, 2026): Fixed critical bug where the work order completion date entered in Part B of the form was being replaced with the approval date. Root cause: Frontend sends `dateOfCompletion` in DD-MM-YYYY format, but (1) the database column is `completionDateTime`, (2) Luxon's DateTime.fromISO rejects DD-MM-YYYY format, causing fallback to today's date. Fix: (1) Added `normalizeDateToISO` helper to convert DD-MM-YYYY to YYYY-MM-DD with support for legacy formats, (2) Map `dateOfCompletion → completionDateTime` (with ISO timestamp) in PATCH handler while preserving original format for UI stability, (3) Updated approval logic to prioritize `completionDateTime` FIRST using returned workOrder object to avoid race conditions, (4) Graceful error handling - skips maintenance history on malformed dates but continues approval. `completionDateTime` is now the authoritative source for Next Due calculation during Pending to Completion stage. Job.lastDoneDate, maintenance history.dateCompleted, and nextDueDate calculations all correctly use the form's completion date.
