...
-   **Component Deactivation (Soft Delete)**: Sail Admin and Client Admin can deactivate components via a trash button next to "Edit Component". Supports cascade deactivation of child components. Inactive components are hidden from Vessel/HOD roles and shown greyed out with "(Inactive)" label for admin roles. Backend endpoint: `POST /technical/api/components/:id/inactivate`.
