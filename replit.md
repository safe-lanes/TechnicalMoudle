...
## Recent Changes
- **Work Order Completion Date Fix** (Jan 13, 2026): Fixed critical bug where the work order completion date entered in Part B of the form was being replaced with the approval date. Root cause: Frontend sends `dateOfCompletion` but database schema uses `dateCompleted` - the field was never mapped. Fix: (1) Added mapping `dateOfCompletion → dateCompleted` in PATCH handler before persistence, (2) Updated approval logic fallback chain to include `dateOfCompletion`. Now job.lastDoneDate, maintenance history.dateCompleted, and nextDueDate calculations all correctly use the form's completion date.
