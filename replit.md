...
## Recent Changes
- **Defect Form Vessel Master Integration** (Jan 14, 2026): Connected the Defect Form's Vessel dropdown to the external Vessel Master API (same data source as Admin > Masters > Vessel Master). The `useVessels` hook now fetches from `useExternalVessels()` instead of the empty local vessels table. Data mapping handles external API format: `vuid` → vessel ID, `vessel` → vessel name, plus `imoNumber` and `vesselType` for future use. This ensures the Defect Form displays the same vessel list visible in Admin > Masters.
- **Searchable SIRE Hardware Class Combobox** (Jan 14, 2026): Enhanced both Part A Component and Part B2 SIRE Hardware Class dropdowns with searchable combobox interface using shadcn Command/Popover pattern. Key implementation: (1) Reusable SireHardwareClassCombobox component with search/filter capability across all 139 entries. (2) ID-based selection (selectedId prop) correctly handles duplicate Level 3 values; displayValue shows Level 3 text after selection. (3) Database stores unique ID (sireHardwareId/componentHardwareId) plus 3 hierarchy levels for both Part A and Part B2. (4) Part A Component uses same SIRE data temporarily until PMS Module integration provides vessel-specific components. Fields remain independent for future differentiation.
- **SIRE Hardware Class Hierarchical Dropdown** (Jan 13, 2026): Replaced the SFI Code Reference field in Defect Reporting Part B with a new "SIRE Hardware Class" dropdown. Implementation features: (1) Data file with 139 entries from SIRE 2.0 Annex 1, each with unique ID (SHC001-SHC139) and level1/level2/level3 properties. (2) Dropdown displays full path (Level1 > Level2 > Level3) during selection for context, but shows only Level 3 text after selection. (3) Three database columns (sireHardwareLevel1, sireHardwareLevel2, sireHardwareLevel3) store the text values for analytics aggregation. (4) Helper functions (findHardwareClassById, getDefectCountsByLevel1) support defect analytics by hardware category. The unique ID system prevents duplicate Level 3 value collisions.
- **Component Code Auto-Correction** (Jan 12, 2026): Fixed data integrity issue where work orders and jobs could be created with incorrect component codes (e.g., component name "El. Motor No.02" but code stored as "721.801.01" instead of "721.801.02"). This caused maintenance history records to be linked to wrong components. Added auto-correction logic in POST /work-orders, PATCH /work-orders/:id, and POST /jobs endpoints. The system now uses a 3-step lookup (by component ID, by code, by name) to find the component and automatically uses the correct code from the database regardless of what was passed in. Warnings are logged when mismatches are detected and corrected.
