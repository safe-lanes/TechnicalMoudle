...
- **Identity Restructuring (Components, Jobs, Vessels — Complete; Work Orders — Phase 2 of 4)**: Implemented UUID-based canonical identifiers (`cuuid`, `juuid`, `vuuid`) for Components, Jobs, and Vessels, alongside serial integer primary keys. These three entities have fully completed all 4 phases (UUID column, FK constraints, code refactoring, id TEXT→SERIAL conversion). Work Orders (`wouuid`) has completed Phase 1 (UUID column, migration 0028) and Phase 2 (FK constraints on 4 child tables, migrations 0029-0030) but `work_orders.id` is still TEXT PRIMARY KEY — Phase 3 (code refactoring) and Phase 4 (id conversion to SERIAL) are pending.
