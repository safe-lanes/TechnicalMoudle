...
-   **AES-Encrypted LocalStorage (Secure Auth Storage)**: All auth-related data is stored in localStorage using AES encryption via `crypto-js`. Only 5 keys are allowed: `userProfile`, `userRole`, `userType`, `credentials`, `Role_Access_Data`. The encryption utility (`client/src/utils/secureStorage.ts`) enforces strict key validation — any attempt to use an unauthorized key throws an error. The secret key is sourced from `VITE_STORAGE_SECRET` env var. Legacy keys (`currentUser`, `sail_ui_role`) have been completely removed. The `localStorageAnalyzer.ts` utility can detect and decrypt AES values for dev diagnostics. The `roleAccessData.ts` utility generates granular permission matrices (module-level and action-level) based on user role, stored encrypted in `Role_Access_Data`. AuthContext and UIRoleContext both read/write exclusively through the secure storage layer.
