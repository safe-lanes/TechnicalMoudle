...
- **Vessel Identity Architecture**: The `vessels` table uses a dual-identifier approach: `id` (TEXT, primary key) stores the external UUID from the sync API and is referenced by all 31 FK tables, while `vuuid` (TEXT, UNIQUE) stores the same external UUID as a dedicated cross-system sync identifier. The sync-masters endpoint writes the external UUID to both columns. Future consideration: a staged migration to integer auto-increment PK would require updating all 31 FK tables, 60 server files, and 89 client files — this is deferred until a comprehensive migration plan is developed. Impact analysis completed 2026-02-11.
  - **Phase 4 Integer ID Infrastructure (completed 2026-02-11)**: Added `id_int` (SERIAL, UNIQUE) column to `vessels` table and `vessel_id_int` (INTEGER, nullable) to all 31 FK tables. All existing data backfilled. PostgreSQL trigger-based dual-write (`sync_vessel_id_int` function + 31 triggers) automatically populates `vessel_id_int` from `vessel_id` on INSERT/UPDATE — no application code changes needed. Drizzle schemas (both `shared/schema.ts` and all `shared/v2/*/schema.ts`) updated with new columns. Rollback script at `migrations/rollback_vessel_id_int.sql`. API contract unchanged — all endpoints continue accepting UUID strings. The integer IDs are available for future internal optimization (faster joins, smaller indexes) while maintaining full backward compatibility.
