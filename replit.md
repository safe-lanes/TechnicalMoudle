...
- **Vessel Identity Restructure (Phase 4 Complete)**: `vessels.id` is now `SERIAL PRIMARY KEY` (integer auto-increment). `vessels.vuuid` is `TEXT NOT NULL UNIQUE` (the canonical UUID identity column). **`vessels.code` must never be used for vessel identity/lookups** — only `vessels.vuuid` serves that purpose (`vessels.code` currently happens to contain the same UUID but is a separate business field). All 31 child tables' `vessel_id TEXT` columns reference `vessels(vuuid)` via FK constraints (migrations 0009-0013). 12 additional tables use `vessel_code` with no FK constraint (future migration needed to replace with `vessel_id` FK to `vessels.vuuid`). Migration 0014 converted `id` from TEXT to SERIAL. **Server**: All ORM queries use `eq(vessels.vuuid, ...)`. `getVessels()` returns `{ id: number, vuuid: string, name, code }`. `createVessel()` auto-generates integer `id`, accepts `vuuid` for UUID. Sync-masters INSERT omits `id` (auto-generated), upserts on `vuuid`. **Frontend**: VesselContext/useVessels Vessel interface has `id: string` (mapped from vuuid) and `vuuid: string`. All 30+ page files using `vessel.id` work without changes since the UUID value is identical. Child tables' `vessel_id` columns remain TEXT referencing `vessels.vuuid` — no child table migration needed.
