...
- **Component Identity Restructure (cuuid Full Migration Complete)**: `components.cuuid` is `TEXT NOT NULL UNIQUE` — the canonical UUID identity column for all relational references. All 1,638 existing rows were populated with `gen_random_uuid()` via migration 0016. New components auto-generate `cuuid` via `crypto.randomUUID()` in all creation paths. The existing `components.id` (TEXT PK, format `COMP-{ts}-{rand}`) remains unchanged as the primary key but is no longer used for relational lookups. **13 child tables have DB-level FK constraints** (`component_id` → `components.cuuid`, ON DELETE RESTRICT, ON UPDATE CASCADE) via migrations 0018-0020. **Full server-side migration**: All ORM queries use `eq(components.cuuid, ...)` — zero remaining `eq(components.id, ...)` patterns. All child table inserts store `component.cuuid` (not `component.id`). Refactored files: `postgresStorage.ts` (24+ locations), `routes.ts` (12 locations), `runningHoursRoutes.ts` (2 locations), `routes/bulk.ts` (23 locations), all V2 repositories (components, running-hours, work-orders, jobs), all V2 services. Schema defined in both `shared/schema.ts` and all V2 schemas. `cuuid` is omitted from insert schemas (auto-generated).
