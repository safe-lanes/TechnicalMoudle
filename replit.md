...
- **Certificate ID Format (January 2026)**: Certificate Master IDs use prefixes to distinguish origin and prevent conflicts:
  ...
  - **Vessel certificates** (future): Format `VES-{seq}` (e.g., `VES-001`) - vessel-specific additions
  This prefixed ID system ensures no conflicts between Master, Company, and Vessel-added certificates. When a company adds a new certificate, the system auto-generates the next `CMP-` ID and automatically creates applicability records for all vessels with `is_applicable=true`.
