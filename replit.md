...
## Checkpoints
- **Epsilon** (Jan 9, 2026): RH vessel isolation fix. Running hours inheritance now enforced per-vessel. All 4 RH cascade paths (getInheritedComponents, updateMasterRunningHours, setComponentRunningHours, updateRunningHoursBulk) filter inherited components by master's vesselId. Safeguards skip cascade with warnings if vesselId cannot be determined, preventing cross-vessel data leaks when same component codes exist across fleet.
