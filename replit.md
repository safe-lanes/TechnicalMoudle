...
- **Grace Period Logic**: Spec-compliant grace period framework implemented in `shared/workOrders/status.ts`: Calendar jobs get grace until end of month OR 7 days after due date (whichever is longer). Running Hours jobs get 168-hour grace. Enhancement required: Hydrate component currentRH, job dueRH, and maintenanceBasis at call sites for full RH status computation.
- **Job Cycle Updates on WO Approval**: Automatic updates to parent job fields when work orders are completed. Calendar jobs (fully working): `lastDoneDate` and recalculated `nextDueDate` using `calculateNextDueDate` utility (formula: lastDoneDate + frequencyValue × frequencyUnit). Running Hours jobs (framework complete, requires execution data field mapping): Logic exists to update `lastDoneRH` and recalculate `nextDueRH` using currentReading from execution data.
- **Part B Section B3 (Running Hours)**: Work Order form displays Previous Reading (read-only, auto-filled from component's currentCumulativeRH) and Current Reading (editable). Includes caption explaining automatic RH updates on approval: parent component RH set to current reading, child components receive delta propagation.
