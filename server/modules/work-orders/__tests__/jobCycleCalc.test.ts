...
describe('computeJobCycleUpdates', () => {
  it('Calendar: sets lastDoneDate and computes nextDueDate', () => {
    ...
    expect(linkUpdates.lastDoneDate).toBe('01-Aug-2026');
  });
  ...
  it('Running Hours: numeric jobUpdates, string linkUpdates, interval from intervalRunningHour', () => {
    const { jobUpdates, linkUpdates } = computeJobCycleUpdates({
    ...
    expect(linkUpdates.lastDoneRH).toBe('12000');
    expect(linkUpdates.nextDueRH).toBe('12500');
  });
