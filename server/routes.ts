...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/technical/api/reports/due-jobs-7-days", async (req, res) => {
    try {
      ...
      const worksheet = workbook.addWorksheet('Due Jobs (7 Days)', {
        views: [{ state: 'frozen', ySplit: 7, xSplit: 3 }] // Freeze header row and first 3 columns
      });
      
      // ═══════════════════════════════════════════════════════════════
      // PROFESSIONAL HEADER SECTION (Rows 1-6)
      // ═══════════════════════════════════════════════════════════════
      
      // Row 1: Main Title (18 columns now = A to R)
      worksheet.mergeCells('A1:R1');
      const titleCell = worksheet.getCell('A1');
      titleCell.value = 'PLANNED MAINTENANCE SYSTEM';
      ...
      subtitleCell.value = 'DUE JOBS REPORT (NEXT 7 DAYS)';
      subtitleCell.font = { size: 14, bold: true, color: { argb: 'FF1F4E78' } };
      ...
      // Row 4: Vessel Name and Report Date
      ...
      worksheet.getCell('P4').value = `Report Date: ${formatDate(reportDate)}`;
      worksheet.getCell('P4').font = { bold: true, size: 11 };
      worksheet.getCell('P4').alignment = { horizontal: 'right' };
      worksheet.getRow(4).height = 18;
      
      // Row 5: Generated By and Total Jobs
      worksheet.getCell('A5').value = 'Generated By: PMS System';
      worksheet.getCell('A5').font = { size: 10, color: { argb: 'FF666666' } };
      worksheet.mergeCells('P5:R5');
      worksheet.getCell('P5').value = `Total Jobs: ${dueJobs.length}`;
      worksheet.getCell('P5').font = { bold: true, size: 11 };
      worksheet.getCell('P5').alignment = { horizontal: 'right' };
      worksheet.getRow(5).height = 18;
      
      // Row 6: Empty (spacer before table)
      worksheet.getRow(6).height = 10;
      
      // ═══════════════════════════════════════════════════════════════
      // TABLE HEADER ROW (Row 7) - Professional Dark Blue
      // ═══════════════════════════════════════════════════════════════
      
      // Define columns per document requirements: Component Name, Job Title, Assigned To, Due Date, Last Done Date, Criticality, Status
      const columns = [
        { key: 'sno', header: 'S.No', width: 6 },
        { key: 'priority', header: 'Priority', width: 10 },
        { key: 'statusIndicator', header: 'Indicator', width: 10 },
        { key: 'woStatus', header: 'Status', width: 12 },
        { key: 'workOrderNo', header: 'Work Order No', width: 22 },
        { key: 'jobTitle', header: 'Job Title', width: 35 },
        { key: 'componentCode', header: 'Comp Code', width: 14 },
        { key: 'componentName', header: 'Component Name', width: 28 },
        { key: 'department', header: 'Dept', width: 10 },
        { key: 'dueDate', header: 'Due Date', width: 14 },
        { key: 'lastDoneDate', header: 'Last Done', width: 14 },
        { key: 'daysRemaining', header: 'Days Left', width: 10 },
        { key: 'nextDueReading', header: 'Next Due RH', width: 12 },
        { key: 'currentReading', header: 'Current RH', width: 12 },
        { key: 'hoursRemaining', header: 'RH Left', width: 10 },
        { key: 'assignedTo', header: 'Assigned To', width: 18 },
        { key: 'criticality', header: 'Criticality', width: 10 },
        { key: 'critical', header: 'Critical', width: 10 }
      ];
      ...
      // Style header row - Dark Blue (#1F4E78)
      ...
      // ═══════════════════════════════════════════════════════════════
      // DATA ROWS (Starting Row 8) - With alternate row colors
      // ═══════════════════════════════════════════════════════════════
      
      const dataStartRow = 8;
      
      const totalColumns = columns.length; // 18 columns
      
      if (dueJobs.length === 0) {
        const emptyRow = worksheet.getRow(dataStartRow);
        emptyRow.getCell(1).value = 'No jobs due in the next 7 days (including overdue)';
        worksheet.mergeCells(dataStartRow, 1, dataStartRow, totalColumns);
        emptyRow.getCell(1).alignment = { horizontal: 'center', vertical: 'middle' };
        emptyRow.getCell(1).font = { italic: true, color: { argb: 'FF666666' }, size: 11 };
        emptyRow.height = 30;
      } else {
        dueJobs.forEach((job, index) => {
          ...
          // Add warning symbol for overdue jobs
          const jobTitleDisplay = job.isOverdue ? `\u26A0 ${job.jobTitle}` : job.jobTitle;
          const criticalDisplay = job.critical === 'Yes' ? '\u26A0 YES' : 'No';
          
          // Set cell values matching the 18-column layout
          row.getCell(1).value = index + 1; // S.No
          row.getCell(2).value = job.priority; // Priority
          row.getCell(3).value = job.statusIndicator; // Indicator (OVERDUE/URGENT/DUE/ACTIVE)
          row.getCell(4).value = job.woStatus; // Status (from work order)
          row.getCell(5).value = job.workOrderNo; // Work Order No
          row.getCell(6).value = jobTitleDisplay; // Job Title
          row.getCell(7).value = job.componentCode; // Component Code
          row.getCell(8).value = job.componentName; // Component Name
          row.getCell(9).value = job.department; // Department
          row.getCell(10).value = job.dueDate; // Due Date
          row.getCell(11).value = job.lastDoneDate; // Last Done Date
          row.getCell(12).value = job.daysRemaining; // Days Remaining
          row.getCell(13).value = job.nextDueReading; // Next Due RH
          row.getCell(14).value = job.currentReading; // Current RH
          row.getCell(15).value = job.hoursRemaining; // RH Remaining
          row.getCell(16).value = job.assignedTo; // Assigned To
          row.getCell(17).value = job.criticality; // Criticality
          row.getCell(18).value = criticalDisplay; // Critical
          
          row.height = 20;
          
          // Alternate row coloring (white and light gray)
          const isEvenRow = index % 2 === 1;
          const baseColor = isEvenRow ? 'FFF2F2F2' : 'FFFFFFFF'; // Light gray or white
          ...
          row.eachCell((cell, colNum) => {
            ...
            // Alignment based on column type (18 columns now)
            // Numbers: S.No(1), Days Left(12), Next Due RH(13), Current RH(14), RH Left(15)
            if (colNum === 1 || colNum === 12 || colNum === 13 || colNum === 14 || colNum === 15) {
              cell.alignment = { horizontal: 'right', vertical: 'middle' }; // Numbers right-aligned
            } else if (colNum === 10 || colNum === 11) {
              // Dates: Due Date(10), Last Done(11)
              cell.alignment = { horizontal: 'center', vertical: 'middle' }; // Dates centered
            } else {
              cell.alignment = { horizontal: 'left', vertical: 'middle' }; // Text left-aligned
            }
          });
          ...
          const isUrgent = (days <= 2 && days >= 0) || (hours <= 48 && hours >= 0);
          
          // OVERDUE: Red background (#FFC7CE), Dark red text (#9C0006), Bold
          if (job.isOverdue) {
            row.eachCell((cell) => {
              cell.fill = {
                type: 'pattern',
          ...
          // URGENT: Yellow background (#FFF4CE), Dark orange text (#9C6500), Bold
          else if (isUrgent) {
            row.eachCell((cell) => {
              cell.fill = {
                type: 'pattern',
          ...
          // Priority column: Red text for Critical
          if (job.priority === 'Critical') {
            row.getCell(2).font = { bold: true, color: { argb: 'FFFF0000' } };
          }
          
          // Critical column (now column 18): Red text and warning symbol
          if (job.critical === 'Yes') {
            row.getCell(18).font = { bold: true, color: { argb: 'FFFF0000' } };
          }
          
          // Status column styling based on status
          const statusCell = row.getCell(3);
          if (job.statusIndicator === 'OVERDUE') {
            statusCell.font = { bold: true, color: { argb: 'FFFF0000' } };
          } else if (job.statusIndicator === 'URGENT') {
            statusCell.font = { bold: true, color: { argb: 'FFFF6600' } };
          } else if (job.statusIndicator === 'DUE') {
            statusCell.font = { bold: true, color: { argb: 'FF0066CC' } };
          }
        });
      }
      
      // ═══════════════════════════════════════════════════════════════
      // SUMMARY SECTION (After data, skip 2 rows)
      // ═══════════════════════════════════════════════════════════════
      
      const lastDataRowNum = dataStartRow + Math.max(dueJobs.length - 1, 0);
      const summaryStartRow = lastDataRowNum + 3;
      
      // Add thick border line above summary (18 columns now)
      for (let col = 1; col <= totalColumns; col++) {
        const borderCell = worksheet.getRow(summaryStartRow - 1).getCell(col);
        borderCell.border = {
      ...
      // Total Jobs Due
      worksheet.getCell(`A${summaryStartRow + 1}`).value = 'Total Jobs Due:';
      worksheet.getCell(`B${summaryStartRow + 1}`).value = dueJobs.length;
      worksheet.getCell(`B${summaryStartRow + 1}`).font = { bold: true };
      
      // Overdue Jobs
      worksheet.getCell(`A${summaryStartRow + 2}`).value = 'Overdue Jobs:';
      worksheet.getCell(`B${summaryStartRow + 2}`).value = overdueCnt;
      worksheet.getCell(`B${summaryStartRow + 2}`).font = { 
        bold: true, 
        color: overdueCnt > 0 ? { argb: 'FFFF0000' } : { argb: 'FF000000' } 
      };
      
      // Critical Priority Jobs
      worksheet.getCell(`A${summaryStartRow + 3}`).value = 'Critical Priority Jobs:';
      worksheet.getCell(`B${summaryStartRow + 3}`).value = criticalPriorityCnt;
      worksheet.getCell(`B${summaryStartRow + 3}`).font = { bold: true };
      
      // Critical Equipment Jobs
      worksheet.getCell(`A${summaryStartRow + 4}`).value = 'Critical Equipment Jobs:';
      worksheet.getCell(`B${summaryStartRow + 4}`).value = criticalEquipmentCnt;
      worksheet.getCell(`B${summaryStartRow + 4}`).font = { bold: true };
      
      // Urgent Jobs (new)
      worksheet.getCell(`A${summaryStartRow + 5}`).value = 'Urgent Jobs (≤2 days/48 RH):';
      worksheet.getCell(`B${summaryStartRow + 5}`).value = urgentCnt;
      worksheet.getCell(`B${summaryStartRow + 5}`).font = { bold: true };
      ...
      worksheet.pageSetup = {
        ...
        fitToWidth: 1,
        fitToHeight: 0,
        printArea: `A1:R${summaryStartRow + 5}`,
        margins: {
          left: 0.25,
          right: 0.25,
          top: 0.5,
      ...
      // Professional filename: PMS_DueJobs7Days_[VESSELNAME]_[YYYYMMDD]_[HHMM].xlsx
      const now2 = new Date();
      const dateStr = `${now2.getFullYear()}${(now2.getMonth()+1).toString().padStart(2,'0')}${now2.getDate().toString().padStart(2,'0')}`;
      ...
      const filename = `PMS_DueJobs7Days_${safeVesselName}_${dateStr}_${timeStr}.xlsx`;
  ...
  app.post("/technical/api/reports/overdue-jobs", async (req, res) => {
    try {
      ...
      // ═══════════════════════════════════════════════════════════════
      // PROFESSIONAL HEADER SECTION (Rows 1-6) - RED THEME
      // ═══════════════════════════════════════════════════════════════
      ...
      // Row 2: Report Subtitle - RED emphasis
      worksheet.mergeCells('A2:Q2');
      const subtitleCell = worksheet.getCell('A2');
      subtitleCell.value = 'OVERDUE JOBS REPORT';
      subtitleCell.font = { size: 14, bold: true, color: { argb: 'FFC00000' } };
      subtitleCell.alignment = { horizontal: 'center', vertical: 'middle' };
      worksheet.getRow(2).height = 22;
      ...
      worksheet.getCell('O4').value = `Report Date: ${formatDateDisplay(reportDate)}`;
      worksheet.getCell('O4').font = { bold: true, size: 11 };
      worksheet.getCell('O4').alignment = { horizontal: 'right' };
      worksheet.getRow(4).height = 18;
      
      // Row 5: Generated By and Total Overdue (RED if > 0)
      worksheet.getCell('A5').value = 'Generated By: PMS System';
      worksheet.getCell('A5').font = { size: 10, color: { argb: 'FF666666' } };
      worksheet.mergeCells('O5:Q5');
      worksheet.getCell('O5').value = `Total Overdue: ${overdueJobs.length}`;
      worksheet.getCell('O5').font = { 
        bold: true, 
        size: 11, 
        color: overdueJobs.length > 0 ? { argb: 'FFFF0000' } : { argb: 'FF000000' }
      };
      worksheet.getCell('O5').alignment = { horizontal: 'right' };
      worksheet.getRow(5).height = 18;
      ...
      // ═══════════════════════════════════════════════════════════════
      // TABLE HEADER ROW (Row 7) - DARK RED (#C00000)
      // ═══════════════════════════════════════════════════════════════
      ...
      const columns = [
        ...
        { key: 'priority', header: 'Priority', width: 10 },
        { key: 'workOrderNo', header: 'Work Order No', width: 22 },
        { key: 'jobTitle', header: 'Job Title', width: 40 },
        { key: 'componentCode', header: 'Comp Code', width: 15 },
        { key: 'componentName', header: 'Component Name', width: 30 },
        { key: 'department', header: 'Dept', width: 12 },
        { key: 'dueDate', header: 'Due Date', width: 15 },
        { key: 'daysPastDue', header: 'Days Overdue', width: 12 },
        { key: 'nextDueReading', header: 'Next Due RH', width: 14 },
        { key: 'currentReading', header: 'Current RH', width: 14 },
        { key: 'hoursPastDue', header: 'RH Overdue', width: 12 },
        { key: 'overdueType', header: 'Overdue Type', width: 12 },
        { key: 'assignedTo', header: 'Assigned To', width: 20 },
        { key: 'lastDoneDate', header: 'Last Done', width: 15 },
        { key: 'critical', header: 'Critical Equip', width: 10 }
      ];
      
      const totalColumns = columns.length; // 17 columns
      ...
      // Style header row - Dark Red (#C00000) to emphasize urgency
      headerRow.eachCell((cell) => {
        cell.fill = {
          type: 'pattern',
          pattern: 'solid',
          fgColor: { argb: 'FFC00000' } // Dark red
        };
        cell.font = { bold: true, color: { argb: 'FFFFFFFF' }, size: 11 };
        cell.alignment = { vertical: 'middle', horizontal: 'center', wrapText: true };
        cell.border = {
      ...
      // ═══════════════════════════════════════════════════════════════
      // DATA ROWS (Starting Row 8) - With severity-based formatting
      // ═══════════════════════════════════════════════════════════════
      ...
      if (overdueJobs.length === 0) {
        const emptyRow = worksheet.getRow(dataStartRow);
        emptyRow.getCell(1).value = '\u2713 No overdue jobs - All maintenance is current';
        worksheet.mergeCells(dataStartRow, 1, dataStartRow, totalColumns);
        emptyRow.getCell(1).alignment = { horizontal: 'center', vertical: 'middle' };
        emptyRow.getCell(1).font = { italic: true, color: { argb: 'FF008000' }, size: 11 };
        emptyRow.height = 30;
      } else {
        overdueJobs.forEach((job, index) => {
          ...
          // Add warning symbols based on severity
          ...
          // Set cell values
          row.getCell(1).value = index + 1; // S.No
          row.getCell(2).value = job.overdueSeverity; // Severity
          row.getCell(3).value = job.priority; // Priority
          row.getCell(4).value = job.workOrderNo; // Work Order No
          row.getCell(5).value = jobTitleDisplay; // Job Title
          row.getCell(6).value = job.componentCode; // Component Code
          row.getCell(7).value = job.componentName; // Component Name
          row.getCell(8).value = job.department; // Department
          row.getCell(9).value = job.dueDate; // Due Date
          row.getCell(10).value = job.daysPastDue; // Days Overdue
          row.getCell(11).value = job.nextDueReading; // Next Due RH
          row.getCell(12).value = job.currentReading; // Current RH
          row.getCell(13).value = job.hoursPastDue; // RH Overdue
          row.getCell(14).value = job.overdueType; // Overdue Type
          row.getCell(15).value = job.assignedTo; // Assigned To
          row.getCell(16).value = job.lastDoneDate; // Last Done
          row.getCell(17).value = criticalDisplay; // Critical Equipment
          
          row.height = 20;
          
          // Alternate row colors based on severity
          const isEvenRow = index % 2 === 1;
          
          // Apply severity-based styling
          if (job.overdueSeverity === 'CRITICAL') {
            // CRITICAL: Bright Red background, White text, Bold
            row.eachCell((cell) => {
              cell.fill = {
                type: 'pattern',
                pattern: 'solid',
            ...
            // SEVERE: Light Red background, Dark red text, Bold
            row.eachCell((cell) => {
              cell.fill = {
                type: 'pattern',
                pattern: 'solid',
          ...
          } else if (job.overdueSeverity === 'MODERATE') {
            // MODERATE: Light Yellow background, Orange text
            row.eachCell((cell) => {
              cell.fill = {
                type: 'pattern',
                pattern: 'solid',
            ...
            // MINOR: Alternate white/light red
            const baseColor = isEvenRow ? 'FFFFE6E6' : 'FFFFFFFF';
            row.eachCell((cell) => {
              cell.fill = {
                type: 'pattern',
                pattern: 'solid',
          ...
          row.eachCell((cell, colNum) => {
            // Numbers right-aligned: S.No(1), Days Overdue(10), Next Due RH(11), Current RH(12), RH Overdue(13)
            if (colNum === 1 || colNum === 10 || colNum === 11 || colNum === 12 || colNum === 13) {
              cell.alignment = { horizontal: 'right', vertical: 'middle' };
            } else if (colNum === 9 || colNum === 16) {
              // Dates centered: Due Date(9), Last Done(16)
          ...
          if (job.priority === 'High' || job.priority === 'Critical') {
            const currentFont = row.getCell(3).font || {};
            row.getCell(3).font = { ...currentFont, bold: true, color: { argb: 'FFFF0000' } };
          }
          
          // Critical equipment: Red text
          if (job.critical === 'YES') {
            const currentFont = row.getCell(17).font || {};
            row.getCell(17).font = { ...currentFont, bold: true, color: { argb: 'FFFF0000' } };
          }
          
          // Days Overdue conditional formatting
          const daysCell = row.getCell(10);
          if (job.daysPastDue > 30) {
            daysCell.font = { bold: true, color: { argb: 'FFFF0000' } };
          } else if (job.daysPastDue > 14) {
            daysCell.font = { color: { argb: 'FFFF6600' } };
          }
        });
      }
      ...
      // Summary Header - Red text
      worksheet.getCell(`A${summaryStartRow}`).value = 'OVERDUE SUMMARY:';
      worksheet.getCell(`A${summaryStartRow}`).font = { bold: true, size: 12, color: { argb: 'FFFF0000' } };
      
      // Total Overdue Jobs
      worksheet.getCell(`A${summaryStartRow + 1}`).value = 'Total Overdue Jobs:';
      worksheet.getCell(`B${summaryStartRow + 1}`).value = overdueJobs.length;
      worksheet.getCell(`B${summaryStartRow + 1}`).font = { bold: true, color: { argb: 'FFFF0000' } };
      
      // Critical Severity
      worksheet.getCell(`A${summaryStartRow + 2}`).value = 'Critical Severity:';
      worksheet.getCell(`B${summaryStartRow + 2}`).value = debugStats.criticalSeverity;
      worksheet.getCell(`B${summaryStartRow + 2}`).font = { bold: true, color: { argb: 'FFFF0000' } };
      
      // Severe Overdue
      worksheet.getCell(`A${summaryStartRow + 3}`).value = 'Severe Overdue:';
      worksheet.getCell(`B${summaryStartRow + 3}`).value = debugStats.severeSeverity;
      worksheet.getCell(`B${summaryStartRow + 3}`).font = { bold: true, color: { argb: 'FFFF6600' } };
      
      // Overdue Critical Equipment
      worksheet.getCell(`A${summaryStartRow + 4}`).value = 'Overdue Critical Equipment:';
      worksheet.getCell(`B${summaryStartRow + 4}`).value = debugStats.criticalEquipment;
      worksheet.getCell(`B${summaryStartRow + 4}`).font = { bold: true, color: { argb: 'FFFF0000' } };
      
      // Average Days Overdue
      worksheet.getCell(`A${summaryStartRow + 5}`).value = 'Average Days Overdue:';
      worksheet.getCell(`B${summaryStartRow + 5}`).value = avgDaysPastDue;
      worksheet.getCell(`B${summaryStartRow + 5}`).font = { bold: true };
      
      // Longest Overdue
      worksheet.getCell(`A${summaryStartRow + 6}`).value = 'Longest Overdue:';
      worksheet.getCell(`B${summaryStartRow + 6}`).value = `${maxDaysPastDue} days`;
      worksheet.getCell(`B${summaryStartRow + 6}`).font = { bold: true, color: { argb: 'FFFF0000' } };
      
      // Action Required notice
      worksheet.mergeCells(`A${summaryStartRow + 8}:Q${summaryStartRow + 8}`);
      worksheet.getCell(`A${summaryStartRow + 8}`).value = '\u26A0 ACTION REQUIRED: All overdue jobs must be completed or officially postponed with Master\'s approval';
      worksheet.getCell(`A${summaryStartRow + 8}`).font = { bold: true, color: { argb: 'FFFF0000' } };
      worksheet.getCell(`A${summaryStartRow + 8}`).alignment = { horizontal: 'center', vertical: 'middle' };
      ...
      worksheet.pageSetup = {
        ...
        fitToWidth: 1,
        fitToHeight: 0,
        printArea: `A1:Q${summaryStartRow + 8}`,
        margins: {
          left: 0.25,
          right: 0.25,
          top: 0.5,
      ...
      // Professional filename: PMS_OverdueJobs_[VESSELNAME]_[YYYYMMDD]_[HHMM].xlsx
      const now2 = new Date();
      const dateStr = `${now2.getFullYear()}${(now2.getMonth()+1).toString().padStart(2,'0')}${now2.getDate().toString().padStart(2,'0')}`;
      const timeStr = `${now2.getHours().toString().padStart(2,'0')}${now2.getMinutes().toString().padStart(2,'0')}`;
      const safeVesselName = vesselName.replace(/[^a-zA-Z0-9]/g, '_');
      const filename = `PMS_OverdueJobs_${safeVesselName}_${dateStr}_${timeStr}.xlsx`;
