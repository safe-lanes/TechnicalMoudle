...
DOCS: dict[str, tuple[str, str, str, list[tuple[str, list[str]]]]] = {
    ...
    "Technical - Recent Updates & Changed Behaviours (Operational) Notes.docx": (
        ...
        [
            ...
            ("1.1.14.13 How To Create A Work Order", [
                "There are three ways a work order is created, with different triggers and rules: (1) planned work orders are generated by the system from the job schedule; (2) a work order for one job is generated from Components with 'Generate WO'; (3) an unplanned work order is created with '+ Unplanned W.O'. The June 2026 PMS manuals describe only (2) and (3). The ship/office conditions below are implementation-specific: they were read from the application code (repository revision recorded in PROVENANCE.md) and are not stated in the manuals.",
                "1. Planned (scheduled) work orders — generated by the system from each job's schedule (frequency and next due date, see Components Part C, PMS manual §1.1.4 p18 Office / p16 Vessel); they appear under the Scheduled / Due / Overdue tabs (§1.1.5.1 p28). Who and when: ON THE SHIP they are generated automatically by the daily scan, nobody creates them. IN THE OFFICE nothing runs automatically; the only way is 'Generate Now' on the Work Orders screen, and it works ONLY for a Sail Admin AND ONLY when the selected vessel's 'office work-order generation' switch is on ('Lead Time & Grace Period Settings', off by default) — any other office role, or a vessel with the switch off, is refused. [Implementation note: the manuals do not say how these entries are produced.]",
                "2. A work order for one specific job — Components → open the component → Part C (its jobs) → 'Generate WO' → choose the reason (Planning, Breakdown or Other); refused if the job already has an active work order (PMS manual §1.1.4 p18 Office / p16 Vessel). Who and when: ON THE SHIP always available. IN THE OFFICE ONLY when that vessel's 'office work-order generation' switch is on; with the switch off the request is refused. No particular role is required for this button — it is NOT limited to Sail Admin. [Implementation note: the manuals state no office condition.]",
                "3. An unplanned work order (breakdown or other unscheduled maintenance) — created by the user: Work Orders → select the vessel → '+ Unplanned W.O' → fill Part A and click Save (draft) → complete Part B and click Submit Work Order (PMS manual §1.1.5.2 p29 Office / p24 Vessel). Who and when: available on the ship and in the office, no switch involved.",
                "When answering 'how do I create a work order', state all three ways together with their ship/office conditions above; do not present the unplanned form as the only way.",
                "Which one you need: for routine maintenance that is already in the job schedule, do nothing — the work order is generated (way 1) or generate it early for one job (way 2); for work that is not in the schedule, create an unplanned work order (way 3). Numbering of all three follows §1.1.14.1.",
            ]),
        ]),
...
PROVENANCE: list[tuple[str, str, str, str]] = [
    ...
    ("Recent Updates", "R3.2 §1.1.14.13 way 2: per-job 'Generate WO' (Components Part C, reason Planning/Breakdown/Other, duplicate refused); office condition = vessel switch ONLY, no role check on this path (definitive trace 14-Sep: route left open, controller has no role check, service checks only isOfficeWoGenerationEnabled, scanner has none; the Sail-Admin gate evaluateDirectGeneration has exactly one caller = Generate Now)", "ADDED (R3.2)", "client/src/pages/pms/Components.tsx:916,1016-1018; server/modules/jobs/routes.ts:11,40; server/modules/jobs/controllers/jobController.ts generateWorkOrder (no role check); jobService.ts:570-590; server/services/jobDueScanner.ts generateWorkOrderForJob (no gate); workOrderController.ts:131-148 (the only evaluateDirectGeneration caller); PMS manual p18 Office / p16 Vessel"),
