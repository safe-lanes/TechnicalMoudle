...
interface RePostponeWorkOrderDialogProps {
  isOpen: boolean;
  onClose: () => void;
  workOrder: {
    id?: string | null;
    ...
    /** Current approved postponement due date — shown as read-only reference */
    postponementEndDate?: string | null;
    /** Pre-fill: existing requested new date (for Awaiting Office Approval re-postponements) */
...
const RePostponeWorkOrderDialog: React.FC<RePostponeWorkOrderDialogProps> = ({
  isOpen,
  onClose,
  workOrder,
  onConfirm,
  ...
  // The "original due date" for a re-postponement is the currently-approved
  // postponement date (dueDate on the WO at the time of opening), used for
  // min-date and duration calculations.
  const currentApprovedDate = workOrder?.postponementEndDate || workOrder?.dueDate || null;
  ...
  const computedDuration = useMemo(() => {
    if (!formData.postponeDate) return "";
    const orig = parseDateFlexible(currentApprovedDate);
  ...
  }, [formData.postponeDate, currentApprovedDate]);
  ...
  // Soft 90-day advisory warning — measured from the current approved date
  const exceeds90Days = useMemo(() => {
    if (!formData.postponeDate || !currentApprovedDate) return false;
    const orig = parseDateFlexible(currentApprovedDate);
    const postpone = new Date(formData.postponeDate + "T00:00:00");
  ...
  useEffect(() => {
    ...
    const loadDocs = async () => {
      ...
      try {
        ...
        console.error("Failed to load re-postponement docs:", err);
        if (!cancelled) {
          setPostponementDocs([]);
          setRiskAssessmentDoc(null);
        }
  ...
  const handleSubmit = () => {
    ...
    if (!formData.postponeDate) {
      ...
      if (orig && !isNaN(orig.getTime())) {
        ...
        postpone.setHours(0, 0, 0, 0);
        if (postpone <= orig) {
          newErrors.postponeDate = "Postpone date must be after the current approved postponement date.";
        }
      }
    }
  ...
  const currentApprovedDateFormatted = (() => {
    if (!currentApprovedDate) return "—";
    const d = parseDateFlexible(currentApprovedDate);
    return d ? formatDDMMMYYYY(d) : currentApprovedDate;
  })();
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
          ...
          <div className="space-y-3 py-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label htmlFor="re-postpone-workOrderId" className="text-sm">Work Order ID</Label>
                <Input
                  id="re-postpone-workOrderId"
                  value={formData.workOrderId}
                  className="bg-gray-50 h-9"
                  readOnly
                  data-testid="input-re-postpone-wo-id"
                />
              </div>
              <div className="space-y-1">
                <Label htmlFor="re-postpone-component" className="text-sm">Component</Label>
                <Input
                  id="re-postpone-component"
                  value={formData.component}
                  className="bg-gray-50 h-9"
                  readOnly
                  data-testid="input-re-postpone-component"
                />
              </div>
            </div>
            ...
            <div className="space-y-1">
              <Label htmlFor="re-postpone-jobTitle" className="text-sm">Job Title</Label>
              <Input
                id="re-postpone-jobTitle"
                value={formData.jobTitle}
                className="bg-gray-50 h-9"
                readOnly
                data-testid="input-re-postpone-job-title"
              />
            </div>

            {/* Row 3: Current Approved Postponement Date / Risk Assessment */}
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label className="text-sm">Current Approved Postponement Date</Label>
                <Input
                  value={currentApprovedDateFormatted}
                  className="bg-gray-50 h-9"
                  readOnly
                  data-testid="input-re-postpone-current-approved-date"
                />
                <p className="text-xs text-gray-400">The currently-approved postponement end date</p>
              </div>
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="inline-flex items-center h-9 rounded-md border border-gray-200 bg-gray-50 overflow-hidden">
                    <button
                      ...
                      data-testid="button-re-postpone-risk-assessment-upload"
                    ...
                    <button
                      ...
                      data-testid="button-re-postpone-risk-assessment-view"
                  ...
                  <button
                    ...
                    data-testid="button-re-postpone-risk-assessment-link"
                  ...
                  <input
                    ...
                    data-testid="input-file-re-postpone-risk-assessment"
                  />
                </div>
                <p className="text-xs text-gray-500 truncate" data-testid="text-re-postpone-risk-assessment-filename">
                  {riskAssessmentDoc ? (
            ...
            <div className="space-y-1">
              <Label htmlFor="re-postpone-date" className="text-sm">
                New Postpone Date <span className="text-red-500">*</span>
              </Label>
              <Input
                id="re-postpone-date"
                type="date"
                ...
                data-testid="input-re-postpone-date"
              />
              {errors.postponeDate && (
                <p className="text-sm text-red-500" data-testid="error-re-postpone-date">{errors.postponeDate}</p>
              )}
            </div>

            {/* 90-day advisory warning */}
            {exceeds90Days && (
              <div
                className="flex items-start gap-2 rounded-md border border-amber-300 bg-amber-50 px-3 py-2 text-sm text-amber-800"
                data-testid="warning-re-postpone-90-days"
              >
                <span className="mt-0.5 shrink-0 text-amber-500">⚠</span>
                <span>
                  Warning: The selected Re-Postponement Date exceeds the recommended 90-day limit from the current approved postponement date.
                </span>
              </div>
            )}

            {/* Row 5: Reason for Re-Postponement */}
            <div className="space-y-1">
              <Label htmlFor="re-postpone-reason" className="text-sm">
                Reason for Re-Postponement <span className="text-red-500">*</span>
              </Label>
              <Select
              ...
              >
                <SelectTrigger
                  id="re-postpone-reason"
                  data-testid="select-re-postponement-reason"
                  className={errors.reason ? "border-red-500" : ""}
                >
                  <SelectValue placeholder="Select a reason for re-postponement..." />
                </SelectTrigger>
              ...
              {errors.reason && (
                <p className="text-sm text-red-500" data-testid="re-postponement-reason-error">{errors.reason}</p>
              )}
            </div>

            {/* Row 6: Remarks */}
            <div className="space-y-1">
              <Label htmlFor="re-postpone-remarks" className="text-sm">
                {isOtherReason ? (
                  <>Custom Re-Postponement Reason <span className="text-red-500">*</span></>
                ) : (
                  "Remarks / Additional Details (Optional)"
                )}
              </Label>
              <Textarea
                id="re-postpone-remarks"
                data-testid="textarea-re-postponement-remarks"
                ...
                placeholder={
                  isOtherReason
                    ? "Enter custom re-postponement reason..."
                    : "Enter additional remarks or details..."
                }
              />
              {errors.remarks && (
                <p className="text-sm text-red-500" data-testid="re-postponement-remarks-error">{errors.remarks}</p>
              )}
            </div>

            {/* Row 7: Next Due Date / Duration (display-only) */}
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <div
                  ...
                  data-testid="display-re-postpone-next-due-date"
                >
                  {computedNextDueDate || <span className="text-gray-400">—</span>}
                </div>
                <p className="text-xs text-gray-400">Auto-calculated from New Postpone Date</p>
              </div>
              <div className="space-y-1">
                <Label className="text-sm">Re-Postponement Duration</Label>
                <div
                  className="flex items-center h-9 px-3 rounded-md border border-gray-200 bg-gray-50 text-sm text-gray-700"
                  data-testid="display-re-postpone-duration"
                >
                  {computedDuration || <span className="text-gray-400">—</span>}
                </div>
                <p className="text-xs text-gray-400">New Date − Current Approved Date</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1">
                <Label htmlFor="re-postpone-approver" className="text-sm">
                ...
                >
                  <SelectTrigger id="re-postpone-approver" className="h-9" data-testid="select-re-postpone-approver">
            ...
            <div className="space-y-1">
              ...
              <div className="flex flex-col items-start gap-1">
                <Button
                  ...
                  data-testid="button-upload-re-postponement"
                >
                  {isUploading ? (
                ...
                <input
                  ...
                  accept=".pdf,.doc,.docx,.jpg,.jpeg,.png,.xlsx"
                  data-testid="input-file-re-postponement"
                />
                <span className="text-xs text-gray-400" data-testid="text-re-postponement-count">
                  {postponementDocs.length}/{MAX_FILES}
                </span>
              </div>
              {postponementDocs.length > 0 && (
                <div className="mt-2 flex flex-wrap gap-2" data-testid="list-re-postponement-docs">
                  {postponementDocs.map((doc) => (
                    <div
                      ...
                      data-testid={`chip-re-postponement-doc-${doc.id}`}
                    >
                      ...
                      <button
                        ...
                        data-testid={`button-delete-re-postponement-doc-${doc.id}`}
                      >
        ...
        <div className="flex justify-end gap-3 pt-4 border-t flex-shrink-0">
          <Button
            variant="outline"
            onClick={onClose}
            data-testid="button-re-postpone-cancel"
          >
            Cancel
          </Button>
          <Button
            ...
            data-testid="button-re-postpone-confirm"
          >
            Submit Re-Postponement
          </Button>
        </div>
      </DialogContent>
    </Dialog>
