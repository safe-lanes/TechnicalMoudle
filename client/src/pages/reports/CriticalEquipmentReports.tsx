        ...
        <>
          ...
          {filteredReports.length > 0 && (
            <div className="rounded-lg border border-gray-200 dark:border-border overflow-hidden bg-white dark:bg-card">
              <CriticalEquipmentReportListGrid
...
interface CriticalEquipmentReportListGridProps {
  reports: CriticalEquipmentReport[];
...
const CriticalEquipmentReportListGrid: React.FC<CriticalEquipmentReportListGridProps> = ({
  reports, generatingReports, getPriorityColor, isLoading, onPreview, onGenerate,
}) => {
  const columns: ReportColumn[] = useMemo(() => [
    {
      ...
      cellRenderer: (p: any) => (
        <div data-testid={`critical-report-row-${p.data.id}`}>
    ...
    },
    {
      ...
      cellRenderer: (p: any) => {
        const r: CriticalEquipmentReport = p.data;
        return (
  ...
  ], [generatingReports, getPriorityColor, isLoading, onPreview, onGenerate]);
  ...
  return (
    <ReportAgGridTable
      ...
      headerHeight={42}
      rowHeight={64}
      testId="grid-critical-reports-list"
