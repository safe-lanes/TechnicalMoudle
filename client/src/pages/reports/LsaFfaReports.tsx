...
interface LsaFfaReportListGridProps {
  reports: LsaFfaReport[];
...
const LsaFfaReportListGrid: React.FC<LsaFfaReportListGridProps> = ({
  reports, generatingReports, anyLoading, getPriorityColor, onPreview, onGenerate,
}) => {
  const columns: ReportColumn[] = useMemo(() => [
    {
      ...
      cellRenderer: (p: any) => (
        <div data-testid={`lsa-ffa-report-row-${p.data.id}`}>
        ...
        return (
          <div className="flex items-center gap-1">
            ...
            <Button
              ...
              disabled={generatingReports.has(`${r.id}-PDF`) || anyLoading}
            ...
            {r.outputs.includes('Excel') && (
              <Button
                ...
                disabled={generatingReports.has(`${r.id}-Excel`) || anyLoading}
  ...
  ], [generatingReports, anyLoading, getPriorityColor, onPreview, onGenerate]);
  ...
  return (
    <ReportAgGridTable
      ...
      headerHeight={42}
      rowHeight={64}
      testId="grid-lsa-ffa-reports-list"
