...
export default function Admin4Dashboard({ onSubViewChange }: { onSubViewChange?: (isSubView: boolean) => void }) {
  ...
  /* ── helper ── */
  const Card1 = ({
    view, icon: Icon, label, description, colorIcon, colorUnderline, colorBg, testId,
  }: {
    view: ViewType; icon: React.ElementType; label: string; description: string;
    colorIcon: string; colorUnderline: string; colorBg: string; testId: string;
  }) => (
    <button
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between gap-2">
          ...
          <ChevronRight className="h-5 w-5 text-gray-300 group-hover:text-gray-500 transition-colors flex-shrink-0 mt-0.5" />
        </div>
  ...
  const Card2 = ({
    view, icon: Icon, label, description, colorIcon, colorUnderline, colorBg, colorBorder, dotColor, testId,
  }: {
    view: ViewType; icon: React.ElementType; label: string; description: string;
    colorIcon: string; colorUnderline: string; colorBg: string; colorBorder: string; dotColor: string; testId: string;
  }) => (
    <button
      onClick={() => setCurrentView(view)}
      className={`relative overflow-hidden text-left rounded-xl p-6 hover:shadow-md transition-all group border ${colorBorder} ${colorBg}`}
      data-testid={testId}
    >
      <div
      ...
      <div className="relative flex items-start gap-4">
        <div className="p-3 rounded-xl bg-white/70 flex-shrink-0">
  ...
  return (
    <div data-testid="text-master-data-title">
      ...
      {/* ── Section 1: Fleet Level Data (no bar) ── */}
      ...
      <div className="mt-8">
        ...
        <div className="mt-5 grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card2
            ...
            colorBg="bg-blue-50/60" colorBorder="border-blue-100" colorIcon="text-blue-500"
            colorUnderline="bg-blue-400" dotColor="rgba(59,130,246,0.12)"
          ...
          <Card2
            ...
            colorBg="bg-indigo-50/60" colorBorder="border-indigo-100" colorIcon="text-indigo-500"
            colorUnderline="bg-indigo-400" dotColor="rgba(99,102,241,0.12)"
          ...
          <Card2
            ...
            colorBg="bg-amber-50/60" colorBorder="border-amber-100" colorIcon="text-amber-500"
            colorUnderline="bg-amber-400" dotColor="rgba(245,158,11,0.12)"
