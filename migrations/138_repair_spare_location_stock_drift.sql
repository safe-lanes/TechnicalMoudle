-- 138: Repair drift between legacy spares ROB columns and spare_location_stock.
-- (Renumbered from the fork's 136 — that number is taken by shipskart_role_mappings
-- on replit_dev; the migration runner keys by full filename, so a DB that already
-- ran this as "136_repair_..." simply re-runs it under this name — predicate-
...
--
-- SYNC SEMANTICS — LOCAL-ONLY REPAIR: spares and spare_location_stock are
-- BOTH_EDITABLE tables that sync via sync_field_log entries, NOT updated_at
-- snapshots, and this migration deliberately writes without field logs. Each
-- instance (ship AND shore) runs it against its OWN ledger evidence and corrects
-- its OWN database; the corrections do NOT propagate across sync. Where the two
-- sides' evidence differs, a residual cross-side mismatch can remain until the
-- next real (field-logged) edit of the affected rows — accepted and watchlisted.
-- The updated_at stamps below are bookkeeping, not a sync trigger.
-- Idempotent: second run changes 0 rows.
