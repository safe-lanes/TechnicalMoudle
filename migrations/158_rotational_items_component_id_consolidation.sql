-- 158: rotational_items component-link consolidation (upgrade path).
--
-- Migration 156 was amended in place (table introduced in the current rotational-items
...
-- instance where the original 156 ran): backfill component_id from component_cuuid,
...
--
-- Idempotent: a complete no-op on databases created from the amended 156.
...
-- NOTE (Task #366): the index on component_id is no longer created here. Migration 159
-- drops component_id entirely (pure master table); recreating the index on fresh
-- installs (where the amended 156 never adds the column) would fail.
